...
class FT1CollectionAssetManagement;
class FT1CollectionContextMenu;
...
{
	friend class FT1CollectionContextMenu;
public:
	...
	SLATE_BEGIN_ARGS( SCollectionView )
		...
		SLATE_EVENT( FT1OnCollectionSelected, OnCollectionSelected )
...
private:
	...
	struct FCreateCollectionPayload
	{
		...
		explicit FCreateCollectionPayload(FT1CollectionItem::FCollectionCreatedEvent InCollectionCreatedEvent)
			: ParentCollection()
			, OnCollectionCreatedEvent(InCollectionCreatedEvent)
		{
		}

		FCreateCollectionPayload(TOptional<FCollectionNameType> InParentCollection, FT1CollectionItem::FCollectionCreatedEvent InCollectionCreatedEvent)
			: ParentCollection(InParentCollection)
			, OnCollectionCreatedEvent(InCollectionCreatedEvent)
		{
		}
		...
		FT1CollectionItem::FCollectionCreatedEvent OnCollectionCreatedEvent;
	};
	...
	void RenameCollectionItem( const TSharedPtr<FT1CollectionItem>& ItemToRename );

	/** Delete the given collections */
	void DeleteCollectionItems( const TArray<TSharedPtr<FT1CollectionItem>>& ItemsToDelete );
	...
	TSharedRef<ITableRow> GenerateCollectionRow( TSharedPtr<FT1CollectionItem> CollectionItem, const TSharedRef<STableViewBase>& OwnerTable );

	/** Get the tree view children for the given item */
	void GetCollectionItemChildren( TSharedPtr<FT1CollectionItem> InParentItem, TArray< TSharedPtr<FT1CollectionItem> >& OutChildItems ) const;
	...
	bool ValidateDragDropOnCollectionItem(TSharedRef<FT1CollectionItem> CollectionItem, const FGeometry& Geometry, const FDragDropEvent& DragDropEvent, bool& OutIsKnownDragOperation);

	/** Handle dropping something on one of our collections */
	FReply HandleDragDropOnCollectionItem(TSharedRef<FT1CollectionItem> CollectionItem, const FGeometry& Geometry, const FDragDropEvent& DragDropEvent);

	/** Recursively expand the parent items of this collection to ensure that it is visible */
	void ExpandParentItems(const TSharedRef<FT1CollectionItem>& InCollectionItem);
	...
	bool IsCollectionCheckBoxEnabled( TSharedPtr<FT1CollectionItem> CollectionItem ) const;

	/** Whether the check box of the given collection item is currently in a checked state */
	ECheckBoxState IsCollectionChecked( TSharedPtr<FT1CollectionItem> CollectionItem ) const;

	/** Handler for when the checked state of the given collection item check box is changed */
	void OnCollectionCheckStateChanged( ECheckBoxState NewState, TSharedPtr<FT1CollectionItem> CollectionItem );

	/** Handler for collection list selection changes */
	void CollectionSelectionChanged( TSharedPtr< FT1CollectionItem > CollectionItem, ESelectInfo::Type SelectInfo );

	/** Handles focusing a collection item widget after it has been created with the intent to rename */
	void CollectionItemScrolledIntoView( TSharedPtr<FT1CollectionItem> CollectionItem, const TSharedPtr<ITableRow>& Widget );
	...
	bool CollectionNameChangeCommit( const TSharedPtr< FT1CollectionItem >& CollectionItem, const FString& NewName, bool bChangeConfirmed, FText& OutWarningMessage );

	/** Checks if the collection name being committed is valid */
	bool CollectionVerifyRenameCommit(const TSharedPtr< FT1CollectionItem >& CollectionItem, const FString& NewName, const FSlateRect& MessageAnchor, FText& OutErrorMessage);
	...
	static void UpdateCollectionItemStatus( const TSharedRef<FT1CollectionItem>& CollectionItem );
...
private:
	...
	TSharedPtr< STreeView< TSharedPtr<FT1CollectionItem> > > CollectionTreePtr;
	...
	typedef TMap< FCollectionNameType, TSharedPtr<FT1CollectionItem> > FAvailableCollectionsMap;
	...
	TArray< TSharedPtr<FT1CollectionItem> > VisibleRootCollectionItems;

	/** The filter to apply to the available collections */
	typedef TTextFilter<const FT1CollectionItem&> FCollectionItemTextFilter;
	TSharedPtr< FCollectionItemTextFilter > CollectionItemTextFilter;

	/** The context menu logic and data */
	TSharedPtr<class FT1CollectionContextMenu> CollectionContextMenu;
	...
	TSharedPtr<FT1CollectionAssetManagement> QuickAssetManagement;
