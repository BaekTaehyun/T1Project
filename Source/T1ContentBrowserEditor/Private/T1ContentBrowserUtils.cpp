...
namespace T1ContentBrowserUtils
{
...
bool T1ContentBrowserUtils::OpenEditorForAsset(const FString& ObjectPath)
{
	// Load the asset if unloaded
	TArray<UObject*> LoadedObjects;
	TArray<FString> ObjectPaths;
	ObjectPaths.Add(ObjectPath);
	T1ContentBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedObjects);
...
bool T1ContentBrowserUtils::OpenEditorForAsset(UObject* Asset)
{
	if( Asset != NULL )
	{
...
bool T1ContentBrowserUtils::OpenEditorForAsset(const TArray<UObject*>& Assets)
{
	if ( Assets.Num() == 1 )
	{
...
bool T1ContentBrowserUtils::LoadAssetsIfNeeded(const TArray<FString>& ObjectPaths, TArray<UObject*>& LoadedObjects, bool bAllowedToPromptToLoadAssets, bool bLoadRedirects)
{
...
void T1ContentBrowserUtils::GetUnloadedAssets(const TArray<FString>& ObjectPaths, TArray<FString>& OutUnloadedObjects)
{
...
bool T1ContentBrowserUtils::PromptToLoadAssets(const TArray<FString>& UnloadedObjects)
{
...
bool T1ContentBrowserUtils::CanRenameFolder(const FString& InFolderPath)
{
	// Cannot rename folders that are part of a classes or collections root
	return !T1ContentBrowserUtils::IsClassPath(InFolderPath) && !T1ContentBrowserUtils::IsCollectionPath(InFolderPath);
}

bool T1ContentBrowserUtils::CanRenameAsset(const FAssetData& InAssetData)
{
...
void T1ContentBrowserUtils::RenameAsset(UObject* Asset, const FString& NewName, FText& ErrorMessage)
{
...
void T1ContentBrowserUtils::CopyAssets(const TArray<UObject*>& Assets, const FString& DestPath)
{
...
void T1ContentBrowserUtils::MoveAssets(const TArray<UObject*>& Assets, const FString& DestPath, const FString& SourcePath)
{
...
int32 T1ContentBrowserUtils::DeleteAssets(const TArray<UObject*>& AssetsToDelete)
{
	return ObjectTools::DeleteObjects(AssetsToDelete);
}

bool T1ContentBrowserUtils::DeleteFolders(const TArray<FString>& PathsToDelete)
{
	...
	else
	{
		...
		if ( T1ContentBrowserUtils::LoadAssetsIfNeeded(ObjectPaths, LoadedAssets) )
		{
			...
			if ( LoadedAssets.Num() == NumAssetsInPaths )
			{
				...
				const int32 NumAssetsDeleted = T1ContentBrowserUtils::DeleteAssets(ToDelete);
...
bool T1ContentBrowserUtils::DeleteEmptyFolderFromDisk(const FString& InPathToDelete)
{
...
void T1ContentBrowserUtils::GetAssetsInPaths(const TArray<FString>& InPaths, TArray<FAssetData>& OutAssetDataList)
{
...
bool T1ContentBrowserUtils::SavePackages(const TArray<UPackage*>& Packages)
{
...
bool T1ContentBrowserUtils::SaveDirtyPackages()
{
...
TArray<UPackage*> T1ContentBrowserUtils::LoadPackages(const TArray<FString>& PackageNames)
{
...
void T1ContentBrowserUtils::DisplayMessage(const FText& Message, const FSlateRect& ScreenAnchor, const TSharedRef<SWidget>& ParentContent)
{
	SContentBrowserPopup::DisplayMessage(Message, ScreenAnchor, ParentContent);
}

void T1ContentBrowserUtils::DisplayConfirmationPopup(const FText& Message, const FText& YesString, const FText& NoString, const TSharedRef<SWidget>& ParentContent, const FOnClicked& OnYesClicked, const FOnClicked& OnNoClicked)
{
...
bool T1ContentBrowserUtils::CopyFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
...
bool T1ContentBrowserUtils::MoveFolders(const TArray<FString>& InSourcePathNames, const FString& DestPath)
{
	TMap<FString, TArray<UObject*> > SourcePathToLoadedAssets;
	...
	for ( auto PathIt = SourcePathToLoadedAssets.CreateConstIterator(); PathIt; ++PathIt )
	{
		...
		if ( PathIt.Value().Num() > 0 )
		{
			...
			T1ContentBrowserUtils::MoveAssets( PathIt.Value(), Destination, PathIt.Key() );
		}
...
bool T1ContentBrowserUtils::PrepareFoldersForDragDrop(const TArray<FString>& SourcePathNames, TMap< FString, TArray<UObject*> >& OutSourcePathToLoadedAssets)
{
...
void T1ContentBrowserUtils::CopyAssetReferencesToClipboard(const TArray<FAssetData>& AssetsToCopy)
{
...
void T1ContentBrowserUtils::CaptureThumbnailFromViewport(FViewport* InViewport, const TArray<FAssetData>& InAssetsToAssign)
{
...
void T1ContentBrowserUtils::ClearCustomThumbnails(const TArray<FAssetData>& InAssetsToAssign)
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
bool T1ContentBrowserUtils::AssetHasCustomThumbnail( const FAssetData& AssetData )
{
	FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
...
T1ContentBrowserUtils::ECBFolderCategory T1ContentBrowserUtils::GetFolderCategory( const FString& InPath )
{
...
bool T1ContentBrowserUtils::IsEngineFolder( const FString& InPath )
{
...
bool T1ContentBrowserUtils::IsDevelopersFolder( const FString& InPath )
{
...
bool T1ContentBrowserUtils::IsPluginFolder(const FString& InPath, const TArray<TSharedRef<IPlugin>>& InPlugins, EPluginLoadedFrom* OutPluginSource)
{
...
bool T1ContentBrowserUtils::IsPluginFolder(const FString& InPath, EPluginLoadedFrom* OutPluginSource)
{
	return IsPluginFolder(InPath, IPluginManager::Get().GetEnabledPluginsWithContent(), OutPluginSource);
}

bool T1ContentBrowserUtils::IsClassesFolder(const FString& InPath)
{
...
bool T1ContentBrowserUtils::IsLocalizationFolder( const FString& InPath )
{
	return FPackageName::IsLocalizedPackage(InPath);
}

void T1ContentBrowserUtils::GetObjectsInAssetData(const TArray<FAssetData>& AssetList, TArray<UObject*>& OutDroppedObjects)
{	
...
bool T1ContentBrowserUtils::IsValidFolderName(const FString& FolderName, FText& Reason)
{
...
bool T1ContentBrowserUtils::DoesFolderExist(const FString& FolderPath)
{
...
bool T1ContentBrowserUtils::IsEmptyFolder(const FString& FolderPath, const bool bRecursive)
{
	if (T1ContentBrowserUtils::IsClassPath(FolderPath))
	{
...
bool T1ContentBrowserUtils::IsRootDir(const FString& FolderPath)
{
	return IsAssetRootDir(FolderPath) || IsClassRootDir(FolderPath);
}

bool T1ContentBrowserUtils::IsAssetRootDir(const FString& FolderPath)
{
...
bool T1ContentBrowserUtils::IsClassRootDir(const FString& FolderPath)
{
...
FText T1ContentBrowserUtils::GetRootDirDisplayName(const FString& FolderPath)
{
	// Strip off any leading or trailing forward slashes
	// We just want the name without any path separators
...
bool T1ContentBrowserUtils::IsClassPath(const FString& InPath)
{
	static const FString ClassesRootPrefix = TEXT("/Classes_");
	return InPath.StartsWith(ClassesRootPrefix);
}

bool T1ContentBrowserUtils::IsCollectionPath(const FString& InPath, FName* OutCollectionName, ECollectionShareType::Type* OutCollectionShareType)
{
...
void T1ContentBrowserUtils::CountPathTypes(const TArray<FString>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void T1ContentBrowserUtils::CountPathTypes(const TArray<FName>& InPaths, int32& OutNumAssetPaths, int32& OutNumClassPaths)
{
	OutNumAssetPaths = 0;
	OutNumClassPaths = 0;
...
void T1ContentBrowserUtils::CountItemTypes(const TArray<FAssetData>& InItems, int32& OutNumAssetItems, int32& OutNumClassItems)
{
	OutNumAssetItems = 0;
	OutNumClassItems = 0;
...
bool T1ContentBrowserUtils::IsValidPathToCreateNewClass(const FString& InPath)
{
...
bool T1ContentBrowserUtils::IsValidPathToCreateNewFolder(const FString& InPath)
{
...
const TSharedPtr<FLinearColor> T1ContentBrowserUtils::LoadColor(const FString& FolderPath)
{
	auto LoadColorInternal = [](const FString& InPath) -> TSharedPtr<FLinearColor>
	{
		...
		if(FPaths::FileExists(GEditorPerProjectIni))
		{
			...
			{
				FLinearColor Color;
				if(Color.InitFromString(ColorStr) && !Color.Equals(T1ContentBrowserUtils::GetDefaultColor()))
				{
			...
			{
				return PathColors.Add(InPath, MakeShareable(new FLinearColor(T1ContentBrowserUtils::GetDefaultColor())));
			}
		}
...
void T1ContentBrowserUtils::SaveColor(const FString& FolderPath, const TSharedPtr<FLinearColor>& FolderColor, bool bForceAdd)
{
	...
	const bool bRemove = !FolderColor.IsValid() || (!bForceAdd && FolderColor->Equals(T1ContentBrowserUtils::GetDefaultColor()));
...
bool T1ContentBrowserUtils::HasCustomColors( TArray< FLinearColor >* OutColors )
{
	...
	if (FPaths::FileExists(GEditorPerProjectIni))
	{
		...
		for( int32 SectionIndex = 0; SectionIndex < Section.Num(); SectionIndex++ )
		{
			...
			if ( EntryStr.Split( TEXT( "=" ), &PathStr, &ColorStr ) )
			{
				...
				if( CurrentColor.InitFromString( ColorStr ) && !CurrentColor.Equals( T1ContentBrowserUtils::GetDefaultColor() ) )
				{
					bHasCustom = true;
					if ( OutColors )
					{
...
FLinearColor T1ContentBrowserUtils::GetDefaultColor()
{
	// The default tint the folder should appear as
	return FLinearColor::Gray;
}

FText T1ContentBrowserUtils::GetExploreFolderText()
{
	FFormatNamedArguments Args;
...
bool T1ContentBrowserUtils::IsValidObjectPathForCreate(const FString& ObjectPath, FText& OutErrorMessage, bool bAllowExistingAsset)
{
...
bool T1ContentBrowserUtils::IsValidFolderPathForCreate(const FString& InFolderPath, const FString& NewFolderName, FText& OutErrorMessage)
{
	if (!T1ContentBrowserUtils::IsValidFolderName(NewFolderName, OutErrorMessage))
	{
		return false;
	}
	...
	if (T1ContentBrowserUtils::DoesFolderExist(NewFolderPath))
	{
	...
	if (!bDisplayL10N && T1ContentBrowserUtils::IsLocalizationFolder(NewFolderPath))
	{
...
int32 T1ContentBrowserUtils::GetPackageLengthForCooking(const FString& PackageName, bool IsInternalBuild)
{
...
bool T1ContentBrowserUtils::IsValidPackageForCooking(const FString& PackageName, FText& OutErrorMessage)
{
...
void T1ContentBrowserUtils::SyncPackagesFromSourceControl(const TArray<FString>& PackageNames)
{
...
void T1ContentBrowserUtils::SyncPathsFromSourceControl(const TArray<FString>& ContentPaths)
{
...
bool T1ContentBrowserUtils::CanDeleteFromAssetView(TWeakPtr<ST1AssetView> AssetView)
{
...
bool T1ContentBrowserUtils::CanRenameFromAssetView(TWeakPtr<ST1AssetView> AssetView)
{
	TArray< FAssetData > AssetViewSelectedAssets = AssetView.Pin()->GetSelectedAssets();
	TArray< FString > SelectedFolders = AssetView.Pin()->GetSelectedFolders();

	const bool bOneAssetSelected = AssetViewSelectedAssets.Num() == 1 && SelectedFolders.Num() == 0		// A single asset
		&& T1ContentBrowserUtils::CanRenameAsset(AssetViewSelectedAssets[0]);								// Which can be renamed

	const bool bOneFolderSelected = AssetViewSelectedAssets.Num() == 0 && SelectedFolders.Num() == 1	// A single folder
		&& T1ContentBrowserUtils::CanRenameFolder(SelectedFolders[0]);									// Which can be renamed

	return (bOneAssetSelected || bOneFolderSelected) && !AssetView.Pin()->IsThumbnailEditMode();
}

bool T1ContentBrowserUtils::CanDeleteFromPathView(const TArray<FString>& SelectedPaths)
{
	int32 NumAssetPaths, NumClassPaths;
...
bool T1ContentBrowserUtils::CanRenameFromPathView(const TArray<FString>& SelectedPaths)
{
	...
	if (T1ContentBrowserUtils::IsRootDir(SelectedPaths[0]))
	{
		return false;
	}

	// We can't rename *any* folders that belong to class roots
	if (T1ContentBrowserUtils::IsClassPath(SelectedPaths[0]))
	{
		return false;
	}
...
bool T1ContentBrowserUtils::IsFavoriteFolder(const FString& FolderPath)
{
	return FT1ContentBrowserSingleton::Get().FavoriteFolderPaths.Contains(FolderPath);
}

void T1ContentBrowserUtils::AddFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	FT1ContentBrowserSingleton::Get().FavoriteFolderPaths.AddUnique(FolderPath);
}

void T1ContentBrowserUtils::RemoveFavoriteFolder(const FString& FolderPath, bool bFlushConfig /*= true*/)
{
	TArray<FString> FoldersToRemove;
	FoldersToRemove.Add(FolderPath);
	
	// Find and remove any subfolders
	for (const FString& FavoritePath : FT1ContentBrowserSingleton::Get().FavoriteFolderPaths)
	{
	...
	{
		FT1ContentBrowserSingleton::Get().FavoriteFolderPaths.Remove(FolderToRemove);
	}
	if (bFlushConfig)
	{
...
const TArray<FString>& T1ContentBrowserUtils::GetFavoriteFolders()
{
	return FT1ContentBrowserSingleton::Get().FavoriteFolderPaths;
}
