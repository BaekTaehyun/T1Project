...
class FT1FrontendFilter_ShowOtherDevelopers;
 ...
 */
class T1SAssetPicker : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( T1SAssetPicker ){}

		/** A struct containing details about how the asset picker should behave */
		SLATE_ARGUMENT(FT1AssetPickerConfig, AssetPickerConfig)

	SLATE_END_ARGS()

	virtual ~T1SAssetPicker();
	...
	const TSharedPtr<ST1AssetView>& GetAssetView() const { return AssetViewPtr; }
...
private:
	...
	FT1OnPathSelected OnFolderEnteredDelegate;
	...
	TSharedPtr< FT1AssetFilterCollectionType > FilterCollection;
	TSharedPtr< FT1FrontendFilter_Text > TextFilter;
	TSharedPtr< FT1FrontendFilter_ShowOtherDevelopers > OtherDevelopersFilter;

	EAssetTypeCategories::Type DefaultFilterMenuExpansion;

	/** The sources data currently used by the picker */
	FT1SourcesData CurrentSourcesData;
