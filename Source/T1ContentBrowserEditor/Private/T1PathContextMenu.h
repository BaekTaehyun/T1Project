...
//#include "Editor/T1ContentBrowser/Private/T1NewAssetOrClassContextMenu.h"
...
class FT1PathContextMenu : public TSharedFromThis<FT1PathContextMenu>
{
public:
	/** Constructor */
	FT1PathContextMenu(const TWeakPtr<SWidget>& InParentContent);

	/** Sets the handler for when new assets are requested */
	void SetOnNewAssetRequested(const FT1NewAssetOrClassContextMenu::FOnNewAssetRequested& InOnNewAssetRequested);

	/** Sets the handler for when new classes are requested */
	void SetOnNewClassRequested(const FT1NewAssetOrClassContextMenu::FOnNewClassRequested& InOnNewClassRequested);

	/** Sets the handler for when importing an asset is requested */
	void SetOnImportAssetRequested( const FT1NewAssetOrClassContextMenu::FOnImportAssetRequested& InOnImportAssetRequested );
...
private:
	...
	FT1NewAssetOrClassContextMenu::FOnNewClassRequested OnNewClassRequested;
