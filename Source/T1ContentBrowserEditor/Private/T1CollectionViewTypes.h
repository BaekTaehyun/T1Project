...
enum class ET1CollectionItemStatus : uint8
{
...
struct FT1CollectionItem
{
	struct FCompareFCollectionItemByName
	{
		FORCEINLINE bool operator()( const TSharedPtr<FT1CollectionItem>& A, const TSharedPtr<FT1CollectionItem>& B ) const
		{
	...
	TWeakPtr<FT1CollectionItem> ParentCollection;

	/** Array of pointers to our child collections (if any) */
	TArray<TWeakPtr<FT1CollectionItem>> ChildCollections;
	...
	ET1CollectionItemStatus CurrentStatus;
	...
	/** Constructor */
	FT1CollectionItem(const FName& InCollectionName, const ECollectionShareType::Type InCollectionType)
		...
		, CurrentStatus(ET1CollectionItemStatus::IsUpToDateAndEmpty)
	{}
};
