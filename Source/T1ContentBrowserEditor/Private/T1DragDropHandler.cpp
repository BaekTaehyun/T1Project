...
bool T1DragDropHandler::ValidateDragDropOnAssetFolder(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent, const FString& TargetPath, bool& OutIsKnownDragOperation)
{
	...
	const bool bIsAssetPath = !T1ContentBrowserUtils::IsClassPath(TargetPath);
	...
	if (Operation->IsOfType<FAssetDragDropOp>())
	{
		...
		T1ContentBrowserUtils::CountItemTypes(DroppedAssets, NumAssetItems, NumClassItems);

		int32 NumAssetPaths, NumClassPaths;
		T1ContentBrowserUtils::CountPathTypes(DroppedAssetPaths, NumAssetPaths, NumClassPaths);
...
void T1DragDropHandler::HandleDropOnAssetFolder(const TSharedRef<SWidget>& ParentWidget, const TArray<FAssetData>& AssetList, const TArray<FString>& AssetPaths, const FString& TargetPath, const FText& TargetDisplayName, FExecuteCopyOrMove CopyActionHandler, FExecuteCopyOrMove MoveActionHandler)
{
	...
	{
		return T1ContentBrowserUtils::IsClassPath(AssetPath);
	});
