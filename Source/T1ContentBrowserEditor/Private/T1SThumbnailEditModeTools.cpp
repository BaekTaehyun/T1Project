...
void ST1ThumbnailEditModeTools::Construct( const FArguments& InArgs, const TSharedPtr<FAssetThumbnail>& InAssetThumbnail )
{
	...
	{
		AssetThumbnail.Pin()->OnAssetDataChanged().AddSP(this, &ST1ThumbnailEditModeTools::OnAssetDataChanged);
	}
	...
	[
		...
		[
			SNew(SButton)
			.Visibility(this, &ST1ThumbnailEditModeTools::GetPrimitiveToolsVisibility)
			.ContentPadding(0)
			.ButtonStyle(FEditorStyle::Get(), "ToggleButton")
			.OnClicked(this, &ST1ThumbnailEditModeTools::ChangePrimitive)
			.ToolTipText(LOCTEXT("CyclePrimitiveThumbnailShapes", "Cycle through primitive shape for this thumbnail"))
			.Content()
			[
				SNew(SImage).Image(this, &ST1ThumbnailEditModeTools::GetCurrentPrimitiveBrush)
			]
		]
		+SHorizontalBox::Slot()
		.HAlign(HAlign_Right)
		...
		[
			SNew(SButton)
			.Visibility(this, &ST1ThumbnailEditModeTools::GetPrimitiveToolsResetToDefaultVisibility)
			.ContentPadding(0)
			.ButtonStyle(FEditorStyle::Get(), "ToggleButton")
			.OnClicked(this, &ST1ThumbnailEditModeTools::ResetToDefault)
...
EVisibility ST1ThumbnailEditModeTools::GetPrimitiveToolsVisibility() const
{
	const bool bIsVisible = !bInSmallView && (ConstGetSceneThumbnailInfoWithPrimitive() != NULL);
	return bIsVisible ? EVisibility::Visible : EVisibility::Collapsed;
}

EVisibility ST1ThumbnailEditModeTools::GetPrimitiveToolsResetToDefaultVisibility() const
{
...
const FSlateBrush* ST1ThumbnailEditModeTools::GetCurrentPrimitiveBrush() const
{
...
FReply ST1ThumbnailEditModeTools::ResetToDefault()
{
...
FReply ST1ThumbnailEditModeTools::OnMouseButtonDown( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( AssetThumbnail.IsValid() &&
...
FReply ST1ThumbnailEditModeTools::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( HasMouseCapture() )
	{
...
FReply ST1ThumbnailEditModeTools::OnMouseMove( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
	if ( HasMouseCapture() )
	{
...
FCursorReply ST1ThumbnailEditModeTools::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
...
USceneThumbnailInfo* ST1ThumbnailEditModeTools::GetSceneThumbnailInfo()
{
...
USceneThumbnailInfoWithPrimitive* ST1ThumbnailEditModeTools::GetSceneThumbnailInfoWithPrimitive()
{
	USceneThumbnailInfo* ThumbnailInfo = SceneThumbnailInfo.Get();
	if ( !ThumbnailInfo )
	{
...
USceneThumbnailInfoWithPrimitive* ST1ThumbnailEditModeTools::ConstGetSceneThumbnailInfoWithPrimitive() const
{
	return Cast<USceneThumbnailInfoWithPrimitive>( SceneThumbnailInfo.Get() );
}

EThumbnailPrimType ST1ThumbnailEditModeTools::GetDefaultThumbnailType() const 
{
...
void ST1ThumbnailEditModeTools::OnAssetDataChanged()
{
