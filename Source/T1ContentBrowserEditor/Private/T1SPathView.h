...
struct FT1HistoryData;
struct FT1TreeItem;

typedef TTextFilter< const FString& > FolderTextFilter;

DECLARE_DELEGATE_OneParam(FT1OnAssetTreeSearchBoxChanged, const FText& /*SearchText*/);
DECLARE_DELEGATE_TwoParams(FT1OnAssetTreeSearchBoxCommitted, const FText& /*InSearchText*/, ETextCommit::Type /*InCommitType*/);

/**
 * The tree view of folders which contain content.
 */
class ST1PathView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS( ST1PathView )
	...
	virtual TSharedPtr<FT1TreeItem> AddPath(const FString& Path, bool bUserNamed = false);
	 ...
	 *								  false to force the view to select the explicit Parent folders of each asset 
	 */
	void SyncTo( const FT1ContentBrowserSelection& ItemSelection, const bool bAllowImplicitSync = false );

	/** Finds the item that represents the specified path, if it exists. */
	TSharedPtr<FT1TreeItem> FindItemRecursive(const FString& Path) const;
...
protected:
	...
	void RecursiveExpandParents(const TSharedPtr<FT1TreeItem>& Item);
	...
	virtual TSharedRef<ITableRow> GenerateTreeRow(TSharedPtr<FT1TreeItem> TreeItem, const TSharedRef<STableViewBase>& OwnerTable);

	/** Handles focusing a folder widget after it has been created with the intent to rename */
	void TreeItemScrolledIntoView(TSharedPtr<FT1TreeItem> TreeItem, const TSharedPtr<ITableRow>& Widget);

	/** Handler for tree view selection changes */
	void TreeSelectionChanged(TSharedPtr< FT1TreeItem > TreeItem, ESelectInfo::Type SelectInfo);
	...
	void GetChildrenForTree(TSharedPtr< FT1TreeItem > TreeItem, TArray< TSharedPtr<FT1TreeItem> >& OutChildren);

	/** Handler for when a name was given to a new folder */
	void FolderNameChanged(const TSharedPtr< FT1TreeItem >& TreeItem, const FString& OldPath, const FVector2D& MessageLocation, const ETextCommit::Type CommitType);
	...
	bool IsTreeItemSelected(TSharedPtr<FT1TreeItem> TreeItem) const;
	...
	virtual void TreeAssetsOrPathsDropped(const TArray<FAssetData>& AssetList, const TArray<FString>& AssetPaths, const TSharedPtr<FT1TreeItem>& TreeItem);

	/** Handler for when asset paths are dropped on a tree item */
	void TreeFilesDropped(const TArray<FString>& FileNames, const TSharedPtr<FT1TreeItem>& TreeItem);
...
private:
	...
	TSharedPtr<FT1TreeItem> AddRootItem( const FString& InFolderName );

	/** Handler for recursively expanding/collapsing items in the tree view */
	void SetTreeItemExpansionRecursive( TSharedPtr< FT1TreeItem > TreeItem, bool bInExpansionState );

	/** Handler for tree view expansion changes */
	void TreeExpansionChanged( TSharedPtr< FT1TreeItem > TreeItem, bool bIsExpanded );
	...
	bool FolderAlreadyExists(const TSharedPtr< FT1TreeItem >& TreeItem, TSharedPtr< FT1TreeItem >& ExistingItem);

	/** Removes the supplied folder from the tree. */
	void RemoveFolderItem(const TSharedPtr< FT1TreeItem >& TreeItem);

	/** True if the specified item is expanded in the asset tree */
	bool IsTreeItemExpanded(TSharedPtr<FT1TreeItem> TreeItem) const;
...
protected:
	...
	{
	public:
		FScopedPreventTreeItemChangedDelegate(const TSharedRef<ST1PathView>& InPathView)
			: PathView(InPathView)
		{
	...
	private:
		TSharedRef<ST1PathView> PathView;
	};

	/** The tree view widget */
	TSharedPtr< STreeView< TSharedPtr<FT1TreeItem>> > TreeViewPtr;

	/** The asset tree search box */
	TSharedPtr< SSearchBox > SearchBoxPtr;

	/** The list of folders in the tree */
	TArray< TSharedPtr<FT1TreeItem> > TreeRootItems;
...
*/
class SFavoritePathView : public ST1PathView
{
public:
	/** Constructs this widget with InArgs */
	...
	FT1OnAssetTreeSearchBoxChanged OnFavoriteSearchChanged;

	FT1OnAssetTreeSearchBoxCommitted OnFavoriteSearchCommitted;

	/** Adds nodes to the tree in order to construct the specified path. If bUserNamed is true, the user will name the folder and Path includes the default name. */
	virtual TSharedPtr<FT1TreeItem> AddPath(const FString& Path, bool bUserNamed = false) override;

	/** Updates favorites based on an external change. */
	void FixupFavoritesFromExternalChange(const TArray<struct FT1MovedContentFolder>& MovedFolders);

private:

	virtual TSharedRef<ITableRow> GenerateTreeRow(TSharedPtr<FT1TreeItem> TreeItem, const TSharedRef<STableViewBase>& OwnerTable) override;
