...
#include "T1CollectionContextMenu.h"
...
void GetBasicStrings(const FT1CollectionItem& InCollection, TArray<FString>& OutBasicStrings)
{
	OutBasicStrings.Add(InCollection.CollectionName.ToString());
}

bool TestComplexExpression(const FT1CollectionItem& InCollection, const FName& InKey, const FTextFilterString& InValue, ETextFilterComparisonOperation InComparisonOperation, ETextFilterTextComparisonMode InTextComparisonMode)
{
...
void SCollectionView::Construct( const FArguments& InArgs )
{
	...
	CollectionContextMenu = MakeShareable(new FT1CollectionContextMenu( SharedThis(this) ));
	...
	{
		QuickAssetManagement = MakeShareable(new FT1CollectionAssetManagement());
	}
	...
	TSharedRef< SWidget > BodyContent = SNew(SVerticalBox)
			...
			.FillHeight(1.f)
			[
				SAssignNew(CollectionTreePtr, STreeView< TSharedPtr<FT1CollectionItem> >)
...
void SCollectionView::HandleCollectionRenamed( const FCollectionNameType& OriginalCollection, const FCollectionNameType& NewCollection )
{
	...
	TSharedPtr<FT1CollectionItem> CollectionItem = AvailableCollections.FindRef(OriginalCollection);
...
{
	TSharedPtr<FT1CollectionItem> CollectionItemToUpdate = AvailableCollections.FindRef(Collection);
...
void SCollectionView::UpdateCollectionItemStatus( const TSharedRef<FT1CollectionItem>& CollectionItem )
{
	TOptional<ET1CollectionItemStatus> NewStatus;
		...
		if (CollectionManagerModule.Get().GetCollectionStatusInfo(CollectionItem->CollectionName, CollectionItem->CollectionType, StatusInfo))
		{
			...
			if (StatusInfo.bUseSCC)
			{
				if (StatusInfo.SCCState.IsValid() && StatusInfo.SCCState->IsSourceControlled())
				{
					...
					{
						NewStatus = ET1CollectionItemStatus::IsCheckedOutByAnotherUser;
					}
					else if (StatusInfo.SCCState->IsConflicted())
					{
						NewStatus = ET1CollectionItemStatus::IsConflicted;
					}
					else if (!StatusInfo.SCCState->IsCurrent())
					{
						NewStatus = ET1CollectionItemStatus::IsOutOfDate;
					}
					else if (StatusInfo.SCCState->IsModified())
					{
						NewStatus = ET1CollectionItemStatus::HasLocalChanges;
					}
				}
				else
				{
					NewStatus = ET1CollectionItemStatus::IsMissingSCCProvider;
				}
			}
			...
			if (!NewStatus.IsSet())
			{
				...
				{
					NewStatus = ET1CollectionItemStatus::IsUpToDateAndEmpty;
				}
				else
				{
					NewStatus = ET1CollectionItemStatus::IsUpToDateAndPopulated;
				}
			}
		}
	}

	CollectionItem->CurrentStatus = NewStatus.Get(ET1CollectionItemStatus::IsUpToDateAndEmpty);
}
...
void SCollectionView::UpdateCollectionItems()
{
	struct FGatherCollectionItems
	{
		...
		void GatherChildCollectionItems(const TSharedPtr<FT1CollectionItem>& InParentCollectionItem, FAvailableCollectionsMap& OutAvailableCollections)
		{
		...
		void ProcessGatheredCollectionsAndRecurse(const TArray<FCollectionNameType>& InCollections, const TSharedPtr<FT1CollectionItem>& InParentCollectionItem, FAvailableCollectionsMap& OutAvailableCollections)
		{
			for (const FCollectionNameType& Collection : InCollections)
			{
				...
				TSharedRef<FT1CollectionItem> CollectionItem = MakeShareable(new FT1CollectionItem(Collection.Name, Collection.Type));
...
void SCollectionView::UpdateFilteredCollectionItems()
{
	...
	auto AddVisibleCollection = [&](const TSharedPtr<FT1CollectionItem>& InCollectionItem)
	{
	...
	auto AddVisibleCollectionRecursive = [&](const TSharedPtr<FT1CollectionItem>& InCollectionItem)
	{
		TSharedPtr<FT1CollectionItem> CollectionItemToAdd = InCollectionItem;
		do
		{
	...
	{
		TArray<TSharedRef<FT1CollectionItem>> CollectionsToExpandTo;

		// Test everything against the filter - a visible child needs to make sure its parents are also marked as visible
		for (const auto& AvailableCollectionInfo : AvailableCollections)
		{
			const TSharedPtr<FT1CollectionItem>& CollectionItem = AvailableCollectionInfo.Value;
		...
		for (const TSharedRef<FT1CollectionItem>& CollectionItem : CollectionsToExpandTo)
		{
			ExpandParentItems(CollectionItem);
		}
	}

	VisibleRootCollectionItems.Sort(FT1CollectionItem::FCompareFCollectionItemByName());
...
void SCollectionView::SetSelectedCollections(const TArray<FCollectionNameType>& CollectionsToSelect, const bool bEnsureVisible)
{
	...
	{
		TSharedPtr<FT1CollectionItem> CollectionItemToSelect = AvailableCollections.FindRef(CollectionToSelect);
...
void SCollectionView::SetExpandedCollections(const TArray<FCollectionNameType>& CollectionsToExpand)
{
	...
	{
		TSharedPtr<FT1CollectionItem> CollectionItemToExpand = AvailableCollections.FindRef(CollectionToExpand);
...
TArray<FCollectionNameType> SCollectionView::GetSelectedCollections() const
{
	...
	TArray<TSharedPtr<FT1CollectionItem>> Items = CollectionTreePtr->GetSelectedItems();
	for ( int32 ItemIdx = 0; ItemIdx < Items.Num(); ++ItemIdx )
	{
		const TSharedPtr<FT1CollectionItem>& Item = Items[ItemIdx];
...
void SCollectionView::ApplyHistoryData( const FT1HistoryData& History )
{	
	// Prevent the selection changed delegate because it would add more history when we are just setting a state
	FScopedPreventSelectionChangedDelegate DelegatePrevention( SharedThis(this) );

	CollectionTreePtr->ClearSelection();
	for (const FCollectionNameType& HistoryCollection : History.T1SourcesData.Collections)
	{
		TSharedPtr<FT1CollectionItem> CollectionHistoryItem = AvailableCollections.FindRef(HistoryCollection);
...
{
	auto SaveCollectionsArrayToIni = [&](const FString& InSubKey, const TArray<TSharedPtr<FT1CollectionItem>>& InCollectionItems)
	{
		FString CollectionsString;

		for (const TSharedPtr<FT1CollectionItem>& CollectionItem : InCollectionItems)
		{
...
void SCollectionView::LoadSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString)
{
	...
	if (NewSelectedCollections.Num() > 0)
	{
		...
		const TArray<TSharedPtr<FT1CollectionItem>> SelectedCollectionItems = CollectionTreePtr->GetSelectedItems();
...
void SCollectionView::MakeSaveDynamicCollectionMenu(FText InQueryString)
{
	...
	TArray<FT1ContentBrowserMenuExtender> MenuExtenderDelegates = ContentBrowserModule.GetAllCollectionViewContextMenuExtenders();
...
void SCollectionView::CreateCollectionItem( ECollectionShareType::Type CollectionType, ECollectionStorageMode::Type StorageMode, const FCreateCollectionPayload& InCreationPayload )
{
	if ( ensure(CollectionType != ECollectionShareType::CST_All) )
	{
		...
		TSharedPtr<FT1CollectionItem> NewItem = MakeShareable(new FT1CollectionItem(CollectionName, CollectionType));
		...
		{
			TSharedPtr<FT1CollectionItem> ParentCollectionItem = AvailableCollections.FindRef(InCreationPayload.ParentCollection.GetValue());
...
void SCollectionView::RenameCollectionItem( const TSharedPtr<FT1CollectionItem>& ItemToRename )
{
...
void SCollectionView::DeleteCollectionItems( const TArray<TSharedPtr<FT1CollectionItem>>& ItemsToDelete )
{
	...
	const TArray<TSharedPtr<FT1CollectionItem>> PreviouslySelectedItems = CollectionTreePtr->GetSelectedItems();

	// Get the first selected item that will be deleted so we can find a suitable new selection
	TSharedPtr<FT1CollectionItem> FirstSelectedItemDeleted;
	...
	{
		TSharedPtr<FT1CollectionItem> RootSelectedItemDeleted = FirstSelectedItemDeleted;
		TSharedPtr<FT1CollectionItem> ParentCollectionItem = FirstSelectedItemDeleted->ParentCollection.Pin();
		...
		if (ParentCollectionItem.IsValid())
		{
			...
			{
				TSharedPtr<FT1CollectionItem> ChildItem = ChildItemWeakPtr.Pin();
	...
	FCollectionManagerModule& CollectionManagerModule = FCollectionManagerModule::GetModule();
	for (const TSharedPtr<FT1CollectionItem>& ItemToDelete : ItemsToDelete)
	{
		...
		else
		{
			...
			T1ContentBrowserUtils::DisplayMessage(
	...
	if (NumSelectedItemsDeleted > 0 || PreviouslySelectedItems.Num() == 0)
	{
		...
		const TArray<TSharedPtr<FT1CollectionItem>> UpdatedSelectedItems = CollectionTreePtr->GetSelectedItems();
...
TSharedRef<ITableRow> SCollectionView::GenerateCollectionRow( TSharedPtr<FT1CollectionItem> CollectionItem, const TSharedRef<STableViewBase>& OwnerTable )
{
	...
	TSharedPtr< STableRow< TSharedPtr<FT1CollectionItem> > > TableRow = SNew( STableRow< TSharedPtr<FT1CollectionItem> >, OwnerTable )
		.OnDragDetected(this, &SCollectionView::OnCollectionDragDetected);

	TableRow->SetContent
		(
			SNew(T1SCollectionTreeItem)
			...
			.IsSelected(TableRow.Get(), &STableRow< TSharedPtr<FT1CollectionItem> >::IsSelectedExclusively)
...
void SCollectionView::GetCollectionItemChildren( TSharedPtr<FT1CollectionItem> InParentItem, TArray< TSharedPtr<FT1CollectionItem> >& OutChildItems ) const
{
	...
	OutChildItems.Sort(FT1CollectionItem::FCompareFCollectionItemByName());
}
...
FReply SCollectionView::HandleDragDropOnCollectionTree(const FGeometry& Geometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (Operation->IsOfType<FCollectionDragDropOp>())
	{
		...
		for (const FCollectionNameType& NewChildCollection : DragDropOp->Collections)
		{
			...
			{
				T1ContentBrowserUtils::DisplayMessage(CollectionManagerModule.Get().GetLastError(), Geometry.GetLayoutBoundingRect(), SharedThis(this));
			}
		}
...
bool SCollectionView::ValidateDragDropOnCollectionItem(TSharedRef<FT1CollectionItem> CollectionItem, const FGeometry& Geometry, const FDragDropEvent& DragDropEvent, bool& OutIsKnownDragOperation)
{
	OutIsKnownDragOperation = false;
...
FReply SCollectionView::HandleDragDropOnCollectionItem(TSharedRef<FT1CollectionItem> CollectionItem, const FGeometry& Geometry, const FDragDropEvent& DragDropEvent)
{
	...
	else if (Operation->IsOfType<FAssetDragDropOp>())
	{
		...
		T1ContentBrowserUtils::DisplayMessage(Message, Geometry.GetLayoutBoundingRect(), SharedThis(this));
...
void SCollectionView::ExpandParentItems(const TSharedRef<FT1CollectionItem>& InCollectionItem)
{
	for (TSharedPtr<FT1CollectionItem> CollectionItemToExpand = InCollectionItem->ParentCollection.Pin(); 
...
bool SCollectionView::IsCollectionCheckBoxEnabled( TSharedPtr<FT1CollectionItem> CollectionItem ) const
{
...
ECheckBoxState SCollectionView::IsCollectionChecked( TSharedPtr<FT1CollectionItem> CollectionItem ) const
{
	if ( QuickAssetManagement.IsValid() )
	{
...
void SCollectionView::OnCollectionCheckStateChanged( ECheckBoxState NewState, TSharedPtr<FT1CollectionItem> CollectionItem )
{
	if ( QuickAssetManagement.IsValid() )
	{
		switch(NewState)
...
void SCollectionView::CollectionSelectionChanged( TSharedPtr< FT1CollectionItem > CollectionItem, ESelectInfo::Type /*SelectInfo*/ )
{
...
void SCollectionView::CollectionItemScrolledIntoView( TSharedPtr<FT1CollectionItem> CollectionItem, const TSharedPtr<ITableRow>& Widget )
{
...
bool SCollectionView::IsCollectionNameReadOnly() const
{
	...
	{
		TArray<TSharedPtr<FT1CollectionItem>> SelectedCollectionItems = CollectionTreePtr->GetSelectedItems();
...
bool SCollectionView::CollectionNameChangeCommit( const TSharedPtr< FT1CollectionItem >& CollectionItem, const FString& NewName, bool bChangeConfirmed, FText& OutWarningMessage )
{
	FCollectionManagerModule& CollectionManagerModule = FCollectionManagerModule::GetModule();
	...
	if ( CollectionItem->bNewCollection )
	{
		...
		TSharedPtr<FT1CollectionItem> ParentCollectionItem = CollectionItem->ParentCollection.Pin();
		if ( ParentCollectionItem.IsValid() )
		{
	...
	else
	{
		...
		if ( !CollectionItemTextFilter->PassesFilter( FT1CollectionItem(NewNameFinal, CollectionItem->CollectionType) ) )
		{
			SearchBoxPtr->SetText(FText::GetEmpty());
		}
...
bool SCollectionView::CollectionVerifyRenameCommit(const TSharedPtr< FT1CollectionItem >& CollectionItem, const FString& NewName, const FSlateRect& MessageAnchor, FText& OutErrorMessage)
{
