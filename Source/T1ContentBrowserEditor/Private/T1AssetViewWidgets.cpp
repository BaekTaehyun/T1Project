...
#include "T1SThumbnailEditModeTools.h"
...
///////////////////////////////
// FT1AssetViewModeUtils
///////////////////////////////

FReply FT1AssetViewModeUtils::OnViewModeKeyDown( const TSet< TSharedPtr<FT1AssetViewItem> >& SelectedItems, const FKeyEvent& InKeyEvent )
{
		...
		for ( auto ItemIt = SelectedItems.CreateConstIterator(); ItemIt; ++ItemIt)
		{
			...
			if(Item.IsValid())
			{
				if(Item->GetType() == EAssetItemType::Folder)
				{
					...
					FARFilter Filter;
					Filter.PackagePaths.Add(*StaticCastSharedPtr<FT1AssetViewFolder>(Item)->FolderPath);
...
TSharedRef<SWidget> FT1AssetViewItemHelper::CreateListItemContents(ST1AssetListItem* const InListItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
{
	return CreateListTileItemContents(InListItem, InThumbnail, OutItemShadowBorder);
}

TSharedRef<SWidget> FT1AssetViewItemHelper::CreateTileItemContents(ST1AssetTileItem* const InTileItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
{
	return CreateListTileItemContents(InTileItem, InThumbnail, OutItemShadowBorder);
}

template <typename T>
TSharedRef<SWidget> FT1AssetViewItemHelper::CreateListTileItemContents(T* const InTileOrListItem, const TSharedRef<SWidget>& InThumbnail, FName& OutItemShadowBorder)
{
	...
	if (InTileOrListItem->IsFolder())
	{
		...
		TSharedPtr<FT1AssetViewFolder> AssetFolderItem = StaticCastSharedPtr<FT1AssetViewFolder>(InTileOrListItem->AssetItem);

		ECollectionShareType::Type CollectionFolderShareType = ECollectionShareType::CST_All;
		if (AssetFolderItem->bCollectionFolder)
		{
			T1ContentBrowserUtils::IsCollectionPath(AssetFolderItem->FolderPath, nullptr, &CollectionFolderShareType);
		}
	...
	else
	{
		...
		ItemContentsOverlay->AddSlot()
		[
			SNew(ST1ThumbnailEditModeTools, InTileOrListItem->AssetThumbnail)
...
class ST1AssetViewItemToolTip : public SToolTip
{
public:
	SLATE_BEGIN_ARGS(ST1AssetViewItemToolTip)
		: _AssetViewItem()
	{ }

		SLATE_ARGUMENT(TSharedPtr<ST1AssetViewItem>, AssetViewItem)
	...
	{
		TSharedPtr<ST1AssetViewItem> AssetViewItemPin = AssetViewItem.Pin();
...
private:
	TWeakPtr<ST1AssetViewItem> AssetViewItem;
};
...
FReply ST1AssetTileView::OnKeyDown( const FGeometry& InGeometry, const FKeyEvent& InKeyEvent )
{
	FReply Reply = FT1AssetViewModeUtils::OnViewModeKeyDown(SelectedItems, InKeyEvent);
	...
	{
		return STileView<TSharedPtr<FT1AssetViewItem>>::OnKeyDown(InGeometry, InKeyEvent);
	}
}

void ST1AssetTileView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	{
		STileView<TSharedPtr<FT1AssetViewItem>>::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}

FReply ST1AssetListView::OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	FReply Reply = FT1AssetViewModeUtils::OnViewModeKeyDown(SelectedItems, InKeyEvent);
	...
	{
		return SListView<TSharedPtr<FT1AssetViewItem>>::OnKeyDown(InGeometry, InKeyEvent);
	}
}

void ST1AssetListView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// Regreshing an asset view is an intensive task. Do not do this while a user
	// is dragging arround content for maximum responsiveness.
	// Also prevents a re-entrancy crash caused by potentially complex thumbnail generators.
	if (!FSlateApplication::Get().IsDragDropping())
	{
		SListView<TSharedPtr<FT1AssetViewItem>>::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}

FReply ST1AssetColumnView::OnKeyDown(const FGeometry& InGeometry, const FKeyEvent& InKeyEvent)
{
	FReply Reply = FT1AssetViewModeUtils::OnViewModeKeyDown(SelectedItems, InKeyEvent);
...
void ST1AssetColumnView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	// Regreshing an asset view is an intensive task. Do not do this while a user
	// is dragging arround content for maximum responsiveness.
	// Also prevents a re-entrancy crash caused by potentially complex thumbnail generators.
	if (!FSlateApplication::Get().IsDragDropping())
	{
		return SListView<TSharedPtr<FT1AssetViewItem>>::Tick(AllottedGeometry, InCurrentTime, InDeltaTime);
	}
}
...
ST1AssetViewItem::~ST1AssetViewItem()
{
...
void ST1AssetViewItem::Construct( const FArguments& InArgs )
{
	...
	SetToolTip(SNew(ST1AssetViewItemToolTip).AssetViewItem(SharedThis(this)));

	SourceControlStateDelay = 0.0f;
	bSourceControlStateRequested = false;

	ISourceControlModule::Get().RegisterProviderChanged(FSourceControlProviderChanged::FDelegate::CreateSP(this, &ST1AssetViewItem::HandleSourceControlProviderChanged));
	SourceControlStateChangedDelegateHandle = ISourceControlModule::Get().GetProvider().RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &ST1AssetViewItem::HandleSourceControlStateChanged));
...
void ST1AssetViewItem::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
TSharedPtr<IToolTip> ST1AssetViewItem::GetToolTip()
{
	return ShouldAllowToolTip.Get() ? SCompoundWidget::GetToolTip() : NULL;
}

bool ST1AssetViewItem::ValidateDragDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent, bool& OutIsKnownDragOperation ) const
{
	OutIsKnownDragOperation = false;
	return IsFolder() && T1DragDropHandler::ValidateDragDropOnAssetFolder(MyGeometry, DragDropEvent, StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath, OutIsKnownDragOperation);
}

void ST1AssetViewItem::OnDragEnter( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
}
	
void ST1AssetViewItem::OnDragLeave( const FDragDropEvent& DragDropEvent )
{
	if(IsFolder())
	{
...
FReply ST1AssetViewItem::OnDragOver( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver); // updates bDraggedOver
	return (bDraggedOver) ? FReply::Handled() : FReply::Unhandled();
}

FReply ST1AssetViewItem::OnDrop( const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent )
{
	if (ValidateDragDrop(MyGeometry, DragDropEvent, bDraggedOver)) // updates bDraggedOver
	{
		...
		if (Operation->IsOfType<FExternalDragOperation>())
		{
			...
			OnFilesDragDropped.ExecuteIfBound(DragDropOp->GetFiles(), StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath);
			return FReply::Handled();
		}
		...
		{
			TSharedPtr<FAssetDragDropOp> DragDropOp = StaticCastSharedPtr<FAssetDragDropOp>(Operation);
			OnAssetsOrPathsDragDropped.ExecuteIfBound(DragDropOp->GetAssets(), DragDropOp->GetAssetPaths(), StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath);
			return FReply::Handled();
		}
	}
...
bool ST1AssetViewItem::IsNameReadOnly() const
{
	...
	if (AssetItem->GetType() != EAssetItemType::Folder)
	{
		...
		return !T1ContentBrowserUtils::CanRenameAsset(StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data);
	}
	else
	{
		// Read-only if we can't be renamed
		return !T1ContentBrowserUtils::CanRenameFolder(StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath);
	}

	return false;
}

void ST1AssetViewItem::HandleBeginNameChange( const FText& OriginalText )
{
	OnRenameBegin.ExecuteIfBound(AssetItem, OriginalText.ToString(), LastGeometry.GetLayoutBoundingRect());
}

void ST1AssetViewItem::HandleNameCommitted( const FText& NewText, ETextCommit::Type CommitInfo )
{
	OnRenameCommit.ExecuteIfBound(AssetItem, NewText.ToString(), LastGeometry.GetLayoutBoundingRect(), CommitInfo);
}

bool  ST1AssetViewItem::HandleVerifyNameChanged( const FText& NewText, FText& OutErrorMessage )
{
...
void ST1AssetViewItem::OnAssetDataChanged()
{
	CachePackageName();
	...
	if ( AssetItem->GetType() != EAssetItemType::Folder )
	{
		UClass* AssetClass = FindObject<UClass>(ANY_PACKAGE, *StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.AssetClass.ToString());
		if (AssetClass)
		{
...
void ST1AssetViewItem::DirtyStateChanged()
{
}

FText ST1AssetViewItem::GetAssetClassText() const
{
	if (!AssetItem.IsValid())
	{
		return FText();
	...
	return FText::FromName(StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.AssetClass);
}

const FSlateBrush* ST1AssetViewItem::GetSCCStateImage() const
{
	return ThumbnailEditMode.Get() ? FEditorStyle::GetNoBrush() : SCCStateBrush;
}

void ST1AssetViewItem::HandleSourceControlProviderChanged(ISourceControlProvider& OldProvider, ISourceControlProvider& NewProvider)
{
	OldProvider.UnregisterSourceControlStateChanged_Handle(SourceControlStateChangedDelegateHandle);
	SourceControlStateChangedDelegateHandle = NewProvider.RegisterSourceControlStateChanged_Handle(FSourceControlStateChanged::FDelegate::CreateSP(this, &ST1AssetViewItem::HandleSourceControlStateChanged));
...
void ST1AssetViewItem::HandleSourceControlStateChanged()
{
...
const FSlateBrush* ST1AssetViewItem::GetDirtyImage() const
{
	return IsDirty() ? AssetDirtyBrush : NULL;
}

EVisibility ST1AssetViewItem::GetThumbnailEditModeUIVisibility() const
{
	return !IsFolder() && ThumbnailEditMode.Get() ? EVisibility::Visible : EVisibility::Collapsed;
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SWidget> ST1AssetViewItem::CreateToolTipWidget() const
{
	if ( AssetItem.IsValid() )
	{
		...
		{
			FAssetData& AssetData = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data;
		...
		{
			const FAssetData& AssetData = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data;
			...
			{
				int32 PackageNameLengthForCooking = T1ContentBrowserUtils::GetPackageLengthForCooking(AssetData.PackageName.ToString(), FEngineBuildSettings::IsInternalBuild());
				...
				AddToToolTipInfoBox(InfoBox, LOCTEXT("TileViewTooltipPathLengthForCookingKey", "Cooking Filepath Length"), FText::Format(LOCTEXT("TileViewTooltipPathLengthForCookingValue", "{0} / {1}"),
					FText::AsNumber(PackageNameLengthForCooking), FText::AsNumber(T1ContentBrowserUtils::MaxCookPathLen)), PackageNameLengthForCooking > T1ContentBrowserUtils::MaxCookPathLen ? true : false);
			}
			else
			{
			...
			OverallTooltipVBox->AddSlot()
				...
				[
					...
					[
						...
						[
							SNew(STextBlock)
							.Visibility(this, &ST1AssetViewItem::GetCheckedOutByOtherTextVisibility)
							.Text(this, &ST1AssetViewItem::GetCheckedOutByOtherText)
		...
		{
			const FText& FolderName = StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderName;
			const FString& FolderPath = StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath;
...
EVisibility ST1AssetViewItem::GetCheckedOutByOtherTextVisibility() const
{
	return GetCheckedOutByOtherText().IsEmpty() ? EVisibility::Collapsed : EVisibility::Visible;
}

FText ST1AssetViewItem::GetCheckedOutByOtherText() const
{
...
FText ST1AssetViewItem::GetAssetUserDescription() const
{
...
void ST1AssetViewItem::AddToToolTipInfoBox(const TSharedRef<SVerticalBox>& InfoBox, const FText& Key, const FText& Value, bool bImportant) const
{
...
void ST1AssetViewItem::UpdatePackageDirtyState()
{
...
bool ST1AssetViewItem::IsDirty() const
{
	return bPackageDirty;
}

void ST1AssetViewItem::UpdateSourceControlState(float InDeltaTime)
{
...
void ST1AssetViewItem::CachePackageName()
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() != EAssetItemType::Folder)
		{
			CachedPackageName = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.PackageName.ToString();
			CachedPackageFileName = SourceControlHelpers::PackageFilename(CachedPackageName);
		}
		else
		{
			CachedPackageName = StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderName.ToString();
		}
	}
}

void ST1AssetViewItem::CacheDisplayTags()
{
...
const FSlateBrush* ST1AssetViewItem::GetBorderImage() const
{
	return bDraggedOver ? FEditorStyle::GetBrush("Menu.Background") : FEditorStyle::GetBrush("NoBorder");
}

bool ST1AssetViewItem::IsFolder() const
{
	return AssetItem.IsValid() && AssetItem->GetType() == EAssetItemType::Folder;
}

FText ST1AssetViewItem::GetNameText() const
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() != EAssetItemType::Folder)
		{
			return FText::FromName(StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.AssetName);
		}
		else
		{
			return StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderName;
		}
	}
...
FSlateColor ST1AssetViewItem::GetAssetColor() const
{
	if(AssetItem.IsValid())
	{
		if(AssetItem->GetType() == EAssetItemType::Folder)
		{
			TSharedPtr<FT1AssetViewFolder> AssetFolderItem = StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem);
			...
			if (AssetFolderItem->bCollectionFolder)
			{
				...
				ECollectionShareType::Type CollectionFolderShareType = ECollectionShareType::CST_All;
				T1ContentBrowserUtils::IsCollectionPath(AssetFolderItem->FolderPath, &CollectionName, &CollectionFolderShareType);

				Color = CollectionViewUtils::LoadColor( CollectionName.ToString(), CollectionFolderShareType );
			}
			else
			{
				Color = T1ContentBrowserUtils::LoadColor( AssetFolderItem->FolderPath );
			}
...
void ST1AssetViewItem::SetForceMipLevelsToBeResident(bool bForce) const
{
...
void ST1AssetViewItem::HandleAssetLoaded(UObject* InAsset) const
{
	if(InAsset != nullptr)
	{
		if(AssetItem.IsValid() && AssetItem->GetType() == EAssetItemType::Normal)
...
bool ST1AssetViewItem::OnVisualizeTooltip(const TSharedPtr<SWidget>& TooltipContent)
{
...
void ST1AssetViewItem::OnToolTipClosing()
{
...
ST1AssetListItem::~ST1AssetListItem()
{
	FCoreUObjectDelegates::OnAssetLoaded.RemoveAll(this);
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void ST1AssetListItem::Construct( const FArguments& InArgs )
{
	ST1AssetViewItem::Construct( ST1AssetViewItem::FArguments()
		.AssetItem(InArgs._AssetItem)
		.OnRenameBegin(InArgs._OnRenameBegin)
		.OnRenameCommit(InArgs._OnRenameCommit)
		.OnVerifyRenameCommit(InArgs._OnVerifyRenameCommit)
	...
	TSharedRef<SWidget> ItemContents = FT1AssetViewItemHelper::CreateListItemContents(this, Thumbnail.ToSharedRef(), ItemShadowBorderName);

	ChildSlot
	[
		SNew(SBorder)
		.BorderImage(this, &ST1AssetViewItem::GetBorderImage)
		.Padding(0)
		.AddMetaData<FTagMetaData>(FTagMetaData(AssetItem->GetType() == EAssetItemType::Normal ? StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.ObjectPath : NAME_None))
		[
			SNew(SHorizontalBox)
			...
			[
				...
				.WidthOverride( this, &ST1AssetListItem::GetThumbnailBoxSize )
				.HeightOverride( this, &ST1AssetListItem::GetThumbnailBoxSize )
				[
					// Drop shadow border
					SNew(SBorder)
					.Padding(4.f)
			...
			[
				...
				[
					...
					.OnVerifyTextChanged(this, &ST1AssetListItem::HandleVerifyNameChanged)
					.HighlightText(InArgs._HighlightText)
					.IsSelected(InArgs._IsSelected)
					.IsReadOnly(this, &ST1AssetListItem::IsNameReadOnly)
				]
	...
	FCoreUObjectDelegates::OnAssetLoaded.AddSP(this, &ST1AssetViewItem::HandleAssetLoaded);
}
END_SLATE_FUNCTION_BUILD_OPTIMIZATION

void ST1AssetListItem::OnAssetDataChanged()
{
	ST1AssetViewItem::OnAssetDataChanged();
...
FOptionalSize ST1AssetListItem::GetThumbnailBoxSize() const
{
	return FOptionalSize( ItemHeight.Get() );
}

FOptionalSize ST1AssetListItem::GetSCCImageSize() const
{
...
ST1AssetTileItem::~ST1AssetTileItem()
{
	FCoreUObjectDelegates::OnAssetLoaded.RemoveAll(this);
}

BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
void ST1AssetTileItem::Construct( const FArguments& InArgs )
{
	ST1AssetViewItem::Construct( ST1AssetViewItem::FArguments()
		.AssetItem(InArgs._AssetItem)
		.OnRenameBegin(InArgs._OnRenameBegin)
	...
	FName ItemShadowBorderName;
	TSharedRef<SWidget> ItemContents = FT1AssetViewItemHelper::CreateTileItemContents(this, Thumbnail.ToSharedRef(), ItemShadowBorderName);
	...
	[
		...
		.BorderImage(this, &ST1AssetViewItem::GetBorderImage)
		.Padding(0)
		.AddMetaData<FTagMetaData>(FTagMetaData((AssetItem->GetType() == EAssetItemType::Normal) ? StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.ObjectPath : ((AssetItem->GetType() == EAssetItemType::Folder) ? FName(*StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath) : NAME_None)))
		[
			SNew(SVerticalBox)
			...
			[
				...
				SNew(SBox)
				.Padding(ThumbnailPadding - 4.f)
				.WidthOverride(this, &ST1AssetTileItem::GetThumbnailBoxSize)
				.HeightOverride( this, &ST1AssetTileItem::GetThumbnailBoxSize )
				[
					// Drop shadow border
					SNew(SBorder)
					.Padding(4.f)
			...
			[
				SAssignNew(InlineRenameWidget, SInlineEditableTextBlock)
					.Font( this, &ST1AssetTileItem::GetThumbnailFont )
					.Text( GetNameText() )
					.OnBeginTextEdit(this, &ST1AssetTileItem::HandleBeginNameChange)
					.OnTextCommitted(this, &ST1AssetTileItem::HandleNameCommitted)
					.OnVerifyTextChanged(this, &ST1AssetTileItem::HandleVerifyNameChanged)
					.HighlightText(InArgs._HighlightText)
					.IsSelected(InArgs._IsSelected)
					.IsReadOnly(this, &ST1AssetTileItem::IsNameReadOnly)
...
void ST1AssetTileItem::OnAssetDataChanged()
{
	ST1AssetViewItem::OnAssetDataChanged();
}

FOptionalSize ST1AssetTileItem::GetThumbnailBoxSize() const
{
	return FOptionalSize( ItemWidth.Get() );
}

FOptionalSize ST1AssetTileItem::GetSCCImageSize() const
{
	return GetThumbnailBoxSize().Get() * 0.2;
}

FSlateFontInfo ST1AssetTileItem::GetThumbnailFont() const
{
...
public:
	...
	void Construct( const FArguments& InArgs, const TSharedRef<ST1AssetColumnItem>& InOwnerAssetColumnItem )
	{
...
private:
	TWeakPtr<ST1AssetViewItem> OwnerAssetColumnItem;
};

void ST1AssetColumnItem::Construct( const FArguments& InArgs )
{
	ST1AssetViewItem::Construct( ST1AssetViewItem::FArguments()
		.AssetItem(InArgs._AssetItem)
		.OnRenameBegin(InArgs._OnRenameBegin)
...
BEGIN_SLATE_FUNCTION_BUILD_OPTIMIZATION
TSharedRef<SWidget> ST1AssetColumnItem::GenerateWidgetForColumn( const FName& ColumnName, FIsSelected InIsSelected )
{
		...
		Content = SNew(SHorizontalBox)
			...
			[
				...
				[
					SNew(SImage)
					.Image( IconBrush )
					.ColorAndOpacity(this, &ST1AssetColumnItem::GetAssetColor)					
				]
				...
				[
					...
					[
						SNew(SImage)
						.Image(this, &ST1AssetColumnItem::GetSCCStateImage)
					]
				]
				...
				[
					...
					[
						SNew(SImage)
						.Image(this, &ST1AssetColumnItem::GetDirtyImage)
					]
				]
			]
			...
			[
				SAssignNew(InlineRenameWidget, SInlineEditableTextBlock)
				.Text( GetNameText() )
				.OnBeginTextEdit(this, &ST1AssetColumnItem::HandleBeginNameChange)
				.OnTextCommitted(this, &ST1AssetColumnItem::HandleNameCommitted)
				.OnVerifyTextChanged(this, &ST1AssetColumnItem::HandleVerifyNameChanged)
				.HighlightText(HighlightText)
				.IsSelected(InIsSelected)
				.IsReadOnly(this, &ST1AssetColumnItem::IsNameReadOnly)
			];
	...
	else if ( ColumnName == "Class" )
	{
		Content = SAssignNew(ClassText, STextBlock)
			.ToolTipText( this, &ST1AssetColumnItem::GetAssetClassText )
			.Text( GetAssetClassText() )
			.HighlightText( HighlightText );
	}
	else if ( ColumnName == "Path" )
	{
		Content = SAssignNew(PathText, STextBlock)
			.ToolTipText( this, &ST1AssetColumnItem::GetAssetPathText )
	...
	else
	{
		Content = SNew(STextBlock)
			.ToolTipText( TAttribute<FText>::Create( TAttribute<FText>::FGetter::CreateSP(this, &ST1AssetColumnItem::GetAssetTagText, ColumnName) ) )
			.Text( TAttribute<FText>::Create( TAttribute<FText>::FGetter::CreateSP(this, &ST1AssetColumnItem::GetAssetTagText, ColumnName) ) );
	}
...
void ST1AssetColumnItem::OnAssetDataChanged()
{
	ST1AssetViewItem::OnAssetDataChanged();
...
FString ST1AssetColumnItem::GetAssetNameToolTipText() const
{
	if ( AssetItem.IsValid() )
	{
		if(AssetItem->GetType() == EAssetItemType::Folder)
		{
			FString Result = StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderName.ToString();
			Result += TEXT("\n");
		...
		{
			const FString AssetName = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.AssetName.ToString();
			const FString AssetType = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.AssetClass.ToString();
...
FText ST1AssetColumnItem::GetAssetPathText() const
{
	if ( AssetItem.IsValid() )
	{
		if(AssetItem->GetType() != EAssetItemType::Folder)
		{
			return FText::FromName(StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem)->Data.PackagePath);
		}
		else
		{
			return FText::FromString(StaticCastSharedPtr<FT1AssetViewFolder>(AssetItem)->FolderPath);
		}
	}
	else
	{
...
FText ST1AssetColumnItem::GetAssetTagText(FName AssetTag) const
{
	if ( AssetItem.IsValid() )
	{
		if(AssetItem->GetType() != EAssetItemType::Folder)
		{
			const TSharedPtr<FT1AssetViewAsset>& ItemAsAsset = StaticCastSharedPtr<FT1AssetViewAsset>(AssetItem);
