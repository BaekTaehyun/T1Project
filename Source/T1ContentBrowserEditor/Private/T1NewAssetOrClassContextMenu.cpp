...
void FT1NewAssetOrClassContextMenu::MakeContextMenu(
	...
	)
{
	...
	T1ContentBrowserUtils::CountPathTypes(InSelectedPaths, NumAssetPaths, NumClassPaths);

	const FString FirstSelectedPath = (InSelectedPaths.Num() > 0) ? InSelectedPaths[0] : FString();
	const bool bIsValidNewClassPath = T1ContentBrowserUtils::IsValidPathToCreateNewClass(FirstSelectedPath);
	const bool bIsValidNewFolderPath = T1ContentBrowserUtils::IsValidPathToCreateNewFolder(FirstSelectedPath);
	...
	if ( InOnGetContentRequested.IsBound() )
	{
		MenuBuilder.BeginSection( "ContentBrowserGetContent", LOCTEXT( "GetContentMenuHeading", "Content" ) );
		{
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon( FEditorStyle::GetStyleSetName(), "T1ContentBrowser.AddContent" ),
				FUIAction( FExecuteAction::CreateStatic( &FT1NewAssetOrClassContextMenu::ExecuteGetContent, InOnGetContentRequested ) )
				);
		}
	...
	if(InOnNewClassRequested.IsBound())
	{
		...
		MenuBuilder.BeginSection("ContentBrowserNewClass", LOCTEXT("ClassMenuHeading", "C++ Class") );
		{
			MenuBuilder.AddMenuEntry(
				...
				FUIAction(
					FExecuteAction::CreateStatic(&FT1NewAssetOrClassContextMenu::ExecuteNewClass, ClassCreationPath, InOnNewClassRequested),
	...
	if (InOnImportAssetRequested.IsBound() && !FirstSelectedPath.IsEmpty())
	{
		MenuBuilder.BeginSection( "ContentBrowserImportAsset", LOCTEXT( "ImportAssetMenuHeading", "Import Asset" ) );
		{
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon( FEditorStyle::GetStyleSetName(), "T1ContentBrowser.ImportIcon" ),
				FUIAction(
					FExecuteAction::CreateStatic( &FT1NewAssetOrClassContextMenu::ExecuteImportAsset, InOnImportAssetRequested, FirstSelectedPath ),
				...
				if (Factories.Num() > 0)
				{
					MenuBuilder.AddSubMenu(
						...
						FNewMenuDelegate::CreateStatic(
							&FT1NewAssetOrClassContextMenu::CreateNewAssetMenuCategory, 
...
void FT1NewAssetOrClassContextMenu::CreateNewAssetMenuCategory(FMenuBuilder& MenuBuilder, EAssetTypeCategories::Type AssetTypeCategory, FString InPath, FOnNewAssetRequested InOnNewAssetRequested, FCanExecuteAction InCanExecuteAction)
{
	...
	for ( auto FactoryIt = FactoriesInThisCategory.CreateConstIterator(); FactoryIt; ++FactoryIt )
	{
		...
		MenuBuilder.AddMenuEntry(
			FUIAction(
				FExecuteAction::CreateStatic( &FT1NewAssetOrClassContextMenu::ExecuteNewAsset, InPath, WeakFactoryClass, InOnNewAssetRequested ),
				InCanExecuteAction
				),
...
void FT1NewAssetOrClassContextMenu::ExecuteImportAsset( FOnImportAssetRequested InOnInportAssetRequested, FString InPath )
{
	InOnInportAssetRequested.ExecuteIfBound( InPath );
}

void FT1NewAssetOrClassContextMenu::ExecuteNewAsset(FString InPath, TWeakObjectPtr<UClass> FactoryClass, FOnNewAssetRequested InOnNewAssetRequested)
{
...
void FT1NewAssetOrClassContextMenu::ExecuteNewClass(FString InPath, FOnNewClassRequested InOnNewClassRequested)
{
...
void FT1NewAssetOrClassContextMenu::ExecuteNewFolder(FString InPath, FOnNewFolderRequested InOnNewFolderRequested)
{
...
void FT1NewAssetOrClassContextMenu::ExecuteGetContent( FOnGetContentRequested InOnGetContentRequested )
{
