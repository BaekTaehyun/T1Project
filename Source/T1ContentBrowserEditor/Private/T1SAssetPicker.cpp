...
T1SAssetPicker::~T1SAssetPicker()
{
	SaveSettings();
}

void T1SAssetPicker::Construct( const FArguments& InArgs )
{
	BindCommands();
	...
	{
		RegisterActiveTimer( 0.f, FWidgetActiveTimerDelegate::CreateSP( this, &T1SAssetPicker::SetFocusPostConstruct ) );
	}
	...
	for (auto DelegateIt = InArgs._AssetPickerConfig.GetCurrentSelectionDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if ((*DelegateIt) != NULL)
		{
			(**DelegateIt) = FT1GetCurrentSelectionDelegate::CreateSP(this, &T1SAssetPicker::GetCurrentSelection);
		}
	}
	...
	for(auto DelegateIt = InArgs._AssetPickerConfig.SyncToAssetsDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if((*DelegateIt) != NULL)
		{
			(**DelegateIt) = FT1SyncToAssetsDelegate::CreateSP(this, &T1SAssetPicker::SyncToAssets);
		}
	}
	...
	for (auto DelegateIt = InArgs._AssetPickerConfig.SetFilterDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if ((*DelegateIt) != NULL)
		{
			(**DelegateIt) = FT1SetARFilterDelegate::CreateSP(this, &T1SAssetPicker::SetNewBackendFilter);
		}
	}
	...
	for (auto DelegateIt = InArgs._AssetPickerConfig.RefreshAssetViewDelegates.CreateConstIterator(); DelegateIt; ++DelegateIt)
	{
		if ((*DelegateIt) != NULL)
		{
			(**DelegateIt) = FT1RefreshAssetViewDelegate::CreateSP(this, &T1SAssetPicker::RefreshAssetView);
		}
	}
	...
	{
		HighlightText = TAttribute< FText >( this, &T1SAssetPicker::GetHighlightedText );

		OtherDevelopersFilter = MakeShareable( new FT1FrontendFilter_ShowOtherDevelopers(nullptr) );
		...
		if(InArgs._AssetPickerConfig.bAddFilterUI)
		{
			...
			[
				...
				.OnGetMenuContent( this, &T1SAssetPicker::MakeAddFilterMenu )
				.HasDownArrow( true )
		...
		[
			...
			.HintText(NSLOCTEXT( "T1ContentBrowser", "SearchBoxHint", "Search Assets" ))
			.OnTextChanged( this, &T1SAssetPicker::OnSearchBoxChanged )
			.OnTextCommitted( this, &T1SAssetPicker::OnSearchBoxCommitted )
			.DelayChangeNotificationsWhileTyping( true )
			.OnKeyDownHandler( this, &T1SAssetPicker::HandleKeyDownFromSearchBox )
		];
		...
		[
			...
			.ToolTipText( this, &T1SAssetPicker::GetShowOtherDevelopersToolTip )
			.OnCheckStateChanged( this, &T1SAssetPicker::HandleShowOtherDevelopersCheckStateChanged )
			...
			+ SVerticalBox::Slot()
				...
				[
					SNew(SButton)
						...
						.OnClicked(this, &T1SAssetPicker::OnNoneButtonClicked)
				]
	...
	CurrentSourcesData = FT1SourcesData(InArgs._AssetPickerConfig.Filter.PackagePaths, InArgs._AssetPickerConfig.Collections);
	...
	if(InArgs._AssetPickerConfig.bAddFilterUI)
	{
		...
		[
			...
			.OnFilterChanged(this, &T1SAssetPicker::OnFilterChanged)
	...
	[
		...
		.OnAssetSelectionChanged(this, &T1SAssetPicker::HandleAssetSelectionChanged)
		.OnAssetsActivated(this, &T1SAssetPicker::HandleAssetsActivated)
		...
		.AreRealTimeThumbnailsAllowed(this, &T1SAssetPicker::IsHovered)
		.FrontendFilters(FrontendFilters)
		...
		.OnPathSelected(this, &T1SAssetPicker::FolderEntered)
		.HiddenColumnNames(InArgs._AssetPickerConfig.HiddenColumnNames)
		.CustomColumns(InArgs._AssetPickerConfig.CustomColumns)
		.OnSearchOptionsChanged(this, &T1SAssetPicker::HandleSearchSettingsChanged)
	];
...
EActiveTimerReturnType T1SAssetPicker::SetFocusPostConstruct( double InCurrentTime, float InDeltaTime )
{
...
FReply T1SAssetPicker::HandleKeyDownFromSearchBox(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply T1SAssetPicker::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
void T1SAssetPicker::FolderEntered(const FString& FolderPath)
{
...
FText T1SAssetPicker::GetHighlightedText() const
{
	return TextFilter->GetRawFilterText();
}

void T1SAssetPicker::SetSearchBoxText(const FText& InSearchText)
{
...
void T1SAssetPicker::OnSearchBoxChanged(const FText& InSearchText)
{
	SetSearchBoxText( InSearchText );
}

void T1SAssetPicker::OnSearchBoxCommitted(const FText& InSearchText, ETextCommit::Type CommitInfo)
{
	SetSearchBoxText( InSearchText );
...
void T1SAssetPicker::SetNewBackendFilter(const FARFilter& NewFilter)
{
...
TSharedRef<SWidget> T1SAssetPicker::MakeAddFilterMenu()
{
	return FilterListPtr->ExternalMakeAddFilterMenu(DefaultFilterMenuExpansion);
}

void T1SAssetPicker::OnFilterChanged()
{
	FARFilter Filter;
...
FReply T1SAssetPicker::OnNoneButtonClicked()
{
...
void T1SAssetPicker::HandleAssetSelectionChanged(const FAssetData& InAssetData, ESelectInfo::Type InSelectInfo)
{
...
void T1SAssetPicker::HandleAssetsActivated(const TArray<FAssetData>& ActivatedAssets, EAssetTypeActivationMethod::Type ActivationMethod)
{
...
void T1SAssetPicker::SyncToAssets(const TArray<FAssetData>& AssetDataList)
{
	AssetViewPtr->SyncToAssets(AssetDataList);
}

TArray< FAssetData > T1SAssetPicker::GetCurrentSelection()
{
	return AssetViewPtr->GetSelectedAssets();
}

void T1SAssetPicker::RefreshAssetView(bool bRefreshSources)
{
	if (bRefreshSources)
	{
		AssetViewPtr->RequestSlowFullListRefresh();
...
FText T1SAssetPicker::GetShowOtherDevelopersToolTip() const
{
...
void T1SAssetPicker::HandleShowOtherDevelopersCheckStateChanged( ECheckBoxState InCheckboxState )
{
	OtherDevelopersFilter->SetShowOtherDeveloperAssets( InCheckboxState == ECheckBoxState::Checked );
}

ECheckBoxState T1SAssetPicker::GetShowOtherDevelopersCheckState() const
{
	return OtherDevelopersFilter->GetShowOtherDeveloperAssets() ? ECheckBoxState::Checked : ECheckBoxState::Unchecked;
}

void T1SAssetPicker::OnRenameRequested() const
{
...
bool T1SAssetPicker::CanExecuteRenameRequested()
{
	TArray< FAssetData > AssetViewSelectedAssets = AssetViewPtr->GetSelectedAssets();
	TArray< FString > SelectedFolders = AssetViewPtr->GetSelectedFolders();
...
void T1SAssetPicker::BindCommands()
{
	Commands = MakeShareable(new FUICommandList);
	// bind commands
	Commands->MapAction( FGenericCommands::Get().Rename, FUIAction(
		FExecuteAction::CreateSP( this, &T1SAssetPicker::OnRenameRequested ),
		FCanExecuteAction::CreateSP( this, &T1SAssetPicker::CanExecuteRenameRequested )
		));
}

void T1SAssetPicker::LoadSettings()
{
	...
	if ( !SettingsString.IsEmpty() )
	{
		...
		if (FilterListPtr.IsValid())
		{
			FilterListPtr->LoadSettings(GEditorPerProjectIni, ST1ContentBrowser::SettingsIniSection, SettingsString);
		}
		
		AssetViewPtr->LoadSettings(GEditorPerProjectIni, ST1ContentBrowser::SettingsIniSection, SettingsString);
	}
}

void T1SAssetPicker::SaveSettings() const
{
	const FString& SettingsString = SaveSettingsName;
	...
	if ( !SettingsString.IsEmpty() )
	{
		...
		if (FilterListPtr.IsValid())
		{
			FilterListPtr->SaveSettings(GEditorPerProjectIni, ST1ContentBrowser::SettingsIniSection, SettingsString);
		}

		AssetViewPtr->SaveSettings(GEditorPerProjectIni, ST1ContentBrowser::SettingsIniSection, SettingsString);
	}
}

void T1SAssetPicker::HandleSearchSettingsChanged()
{
