...
public:
	...
	SLATE_BEGIN_ARGS( SFilter ){}
		...
		SLATE_ARGUMENT( TSharedPtr<FT1FrontendFilter>, FrontendFilter )

		/** Invoked when the filter toggled */
		SLATE_EVENT( T1SFilterList::FT1OnFilterChanged, OnFilterChanged )
	...
	void Construct( const FArguments& InArgs )
	{
		...
		else if ( FrontendFilter.IsValid() )
		{
			...
			FilterToolTip = TAttribute<FText>::Create(TAttribute<FText>::FGetter::CreateSP(FrontendFilter.ToSharedRef(), &FT1FrontendFilter::GetToolTipText));
		}
	...
	/** If this is an front end filter, this is the filter object */
	const TSharedPtr<FT1FrontendFilter>& GetFrontendFilter() const
	{
...
private:
	/** Invoked when the filter toggled */
	T1SFilterList::FT1OnFilterChanged OnFilterChanged;
	...
	/** If this is an front end filter, this is the filter object */
	TSharedPtr<FT1FrontendFilter> FrontendFilter;
...
void T1SFilterList::Construct( const FArguments& InArgs )
{
	...
	TSharedPtr<FT1FrontendFilterCategory> DefaultCategory = MakeShareable( new FT1FrontendFilterCategory(LOCTEXT("FrontendFiltersCategory", "Other Filters"), LOCTEXT("FrontendFiltersCategoryTooltip", "Filter assets by all filters in this category.")) );

	// Add all built-in frontend filters here
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_CheckedOut(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_Modified(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_ShowOtherDevelopers(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_ReplicatedBlueprint(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_ShowRedirectors(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_InUseByLoadedLevels(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_UsedInAnyLevel(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_NotUsedInAnyLevel(DefaultCategory)) );
	AllFrontendFilters.Add( MakeShareable(new FT1FrontendFilter_ArbitraryComparisonOperation(DefaultCategory)) );
	AllFrontendFilters.Add(MakeShareable(new FT1FrontendFilter_Recent(DefaultCategory)));
	...
	//		{
	//			// Grab the filters
	//			TArray< TSharedRef<FT1FrontendFilter> > ExtendedFrontendFilters;
	...
	//			// Grab the categories
	//			for (const TSharedRef<FT1FrontendFilter>& FilterRef : ExtendedFrontendFilters)
	//			{
	//				TSharedPtr<FT1FrontendFilterCategory> Category = FilterRef->GetCategory();
	//				if (Category.IsValid())
	//				{
	...
	{
		TSharedRef<FT1FrontendFilter> Filter = (*Iter);
		TSharedPtr<FT1FrontendFilterCategory> Category = Filter->GetCategory();
		if (Category.IsValid())
		{
			AllFrontendFilterCategories.AddUnique( Category );
		}
...
FReply T1SFilterList::OnMouseButtonUp( const FGeometry& MyGeometry, const FPointerEvent& MouseEvent )
{
...
const TArray<UClass*>& T1SFilterList::GetInitialClassFilters()
{
	return InitialClassFilters;
}

bool T1SFilterList::HasAnyFilters() const
{
	return Filters.Num() > 0;
}

FARFilter T1SFilterList::GetCombinedBackendFilter() const
{
...
TSharedRef<SWidget> T1SFilterList::ExternalMakeAddFilterMenu(EAssetTypeCategories::Type MenuExpansion)
{
	return MakeAddFilterMenu(MenuExpansion);
}

void T1SFilterList::DisableAllFilters()
{
...
void T1SFilterList::RemoveAllFilters()
{
	if ( HasAnyFilters() )
	{
		...
		for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
		{
			...
			else if (FilterToRemove->GetFrontendFilter().IsValid() )
			{
				...
				const TSharedRef<FT1FrontendFilter>& FrontendFilter = FilterToRemove->GetFrontendFilter().ToSharedRef();
...
void T1SFilterList::DisableFiltersThatHideAssets(const TArray<FAssetData>& AssetDataList)
{
	if ( HasAnyFilters() )
	{
		...
		for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
		{
			...
			if ( Filter->IsEnabled() )
			{
				const TSharedPtr<FT1FrontendFilter>& FrontendFilter = Filter->GetFrontendFilter();
				if ( FrontendFilter.IsValid() )
				{
...
void T1SFilterList::SaveSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString) const
{
...
void T1SFilterList::LoadSettings(const FString& IniFilename, const FString& IniSection, const FString& SettingsString)
{
	{
...

	{
		...
		{
			TSharedRef<FT1FrontendFilter>& FrontendFilter = *FrontendFilterIt;
			const FString& FilterName = FrontendFilter->GetName();
...
void T1SFilterList::SetFrontendFilterActive(const TSharedRef<FT1FrontendFilter>& Filter, bool bActive)
{
...
TSharedRef<SFilter> T1SFilterList::AddFilter(const TWeakPtr<IAssetTypeActions>& AssetTypeActions)
{
	TSharedRef<SFilter> NewFilter =
		SNew(SFilter)
		.AssetTypeActions(AssetTypeActions)
		.OnFilterChanged(OnFilterChanged)
		.OnRequestRemove(this, &T1SFilterList::RemoveFilter)
		.OnRequestEnableOnly(this, &T1SFilterList::EnableOnlyThisFilter)
		.OnRequestDisableAll(this, &T1SFilterList::DisableAllFilters)
		.OnRequestRemoveAll(this, &T1SFilterList::RemoveAllFilters);
...
TSharedRef<SFilter> T1SFilterList::AddFilter(const TSharedRef<FT1FrontendFilter>& FrontendFilter)
{
	TSharedRef<SFilter> NewFilter =
		SNew(SFilter)
		.FrontendFilter(FrontendFilter)
		.OnFilterChanged( this, &T1SFilterList::FrontendFilterChanged, FrontendFilter )
		.OnRequestRemove(this, &T1SFilterList::RemoveFilter)
		.OnRequestDisableAll(this, &T1SFilterList::DisableAllFilters)
		.OnRequestRemoveAll(this, &T1SFilterList::RemoveAllFilters);
...
void T1SFilterList::AddFilter(const TSharedRef<SFilter>& FilterToAdd)
{
...
void T1SFilterList::RemoveFilter(const TWeakPtr<IAssetTypeActions>& AssetTypeActions)
{
...
void T1SFilterList::EnableOnlyThisFilter(const TSharedRef<SFilter>& FilterToEnable)
{
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
	{
...
void T1SFilterList::RemoveFilter(const TSharedRef<FT1FrontendFilter>& FrontendFilter)
{
	TSharedPtr<SFilter> FilterToRemove;
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt )
	{
		TSharedPtr<FT1FrontendFilter> Filter = (*FilterIt)->GetFrontendFilter();
...
void T1SFilterList::RemoveFilter(const TSharedRef<SFilter>& FilterToRemove)
{
...
void T1SFilterList::FrontendFilterChanged(TSharedRef<FT1FrontendFilter> FrontendFilter)
{
...
void T1SFilterList::CreateFiltersMenuCategory(FMenuBuilder& MenuBuilder, const TArray<TWeakPtr<IAssetTypeActions>> AssetTypeActionsList) const
{
		...
		if ( WeakTypeActions.IsValid() )
		{
			...
			if ( TypeActions.IsValid() )
			{
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateSP( this, &T1SFilterList::FilterByTypeClicked, WeakTypeActions ),
						FCanExecuteAction(),
						FIsActionChecked::CreateSP(this, &T1SFilterList::IsAssetTypeActionsInUse, WeakTypeActions ) ),
					NAME_None,
...
void T1SFilterList::CreateOtherFiltersMenuCategory(FMenuBuilder& MenuBuilder, TSharedPtr<FT1FrontendFilterCategory> MenuCategory) const
{	
	for ( auto FrontendFilterIt = AllFrontendFilters.CreateConstIterator(); FrontendFilterIt; ++FrontendFilterIt )
	{
		const TSharedRef<FT1FrontendFilter>& FrontendFilter = *FrontendFilterIt;
		...
		if(FrontendFilter->GetCategory() == MenuCategory)
		{
			MenuBuilder.AddMenuEntry(
				...
				FUIAction(
				FExecuteAction::CreateSP( this, &T1SFilterList::FrontendFilterClicked, FrontendFilter ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &T1SFilterList::IsFrontendFilterInUse, FrontendFilter ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
...
TSharedRef<SWidget> T1SFilterList::MakeAddFilterMenu(EAssetTypeCategories::Type MenuExpansion)
{
	...
	MenuBuilder.BeginSection( "ContentBrowserResetFilters" );
	{
		MenuBuilder.AddMenuEntry(
			...
			FSlateIcon(),
			FUIAction( FExecuteAction::CreateSP( this, &T1SFilterList::OnResetFilters ) )
			);
	}
	...
	MenuBuilder.BeginSection(ExpandedCategory->SectionExtensionHook, ExpandedCategory->SectionHeading );
	{
		if(MenuExpansion == EAssetTypeCategories::Basic)
		{
			...
			MenuBuilder.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
				FExecuteAction::CreateSP( this, &T1SFilterList::FilterByTypeCategoryClicked, MenuExpansion ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &T1SFilterList::IsAssetTypeCategoryInUse, MenuExpansion ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
		...
		T1SFilterList::CreateFiltersMenuCategory( MenuBuilder, ExpandedCategory->Assets);
	}
	...
	MenuBuilder.BeginSection("ContentBrowserFilterAdvancedAsset", LOCTEXT("AdvancedAssetsMenuHeading", "Other Assets") );
	{
		if(MenuExpansion == EAssetTypeCategories::Basic)
		{
			...
			for ( auto MenuIt = CategoryToMenuMap.CreateIterator(); MenuIt; ++MenuIt )
			{
				...
				MenuBuilder.AddSubMenu(
					Menu.Name,
					Menu.Tooltip,
					FNewMenuDelegate::CreateSP(this, &T1SFilterList::CreateFiltersMenuCategory, Menu.Assets),
					FUIAction(
					FExecuteAction::CreateSP( this, &T1SFilterList::FilterByTypeCategoryClicked, MenuIt.Key() ),
					FCanExecuteAction(),
					FIsActionChecked::CreateSP(this, &T1SFilterList::IsAssetTypeCategoryInUse, MenuIt.Key() ) ),
					NAME_None,
					EUserInterfaceActionType::ToggleButton
					);
			}
		...
		{
			TSharedPtr<FT1FrontendFilterCategory> Category = (*Iter);

			MenuBuilder.AddSubMenu(
				Category->Title,
				Category->Tooltip,
				FNewMenuDelegate::CreateSP(this, &T1SFilterList::CreateOtherFiltersMenuCategory, Category),
				FUIAction(
				FExecuteAction::CreateSP( this, &T1SFilterList::FrontendFilterCategoryClicked, Category ),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &T1SFilterList::IsFrontendFilterCategoryInUse, Category ) ),
				NAME_None,
				EUserInterfaceActionType::ToggleButton
				);
		}
...
void T1SFilterList::FilterByTypeClicked(TWeakPtr<IAssetTypeActions> AssetTypeActions)
{
	if ( AssetTypeActions.IsValid() )
	{
...
bool T1SFilterList::IsAssetTypeActionsInUse(TWeakPtr<IAssetTypeActions> AssetTypeActions) const
{
	if ( !AssetTypeActions.IsValid() )
	{
		return false;
...
void T1SFilterList::FilterByTypeCategoryClicked(EAssetTypeCategories::Type Category)
{
...
bool T1SFilterList::IsAssetTypeCategoryInUse(EAssetTypeCategories::Type Category) const
{
	TArray<TWeakPtr<IAssetTypeActions>> TypeActionsList;
	GetTypeActionsForCategory(Category, TypeActionsList);
...
void T1SFilterList::GetTypeActionsForCategory(EAssetTypeCategories::Type Category, TArray< TWeakPtr<IAssetTypeActions> >& TypeActions) const
{
...
void T1SFilterList::FrontendFilterClicked(TSharedRef<FT1FrontendFilter> FrontendFilter)
{
	if ( IsFrontendFilterInUse(FrontendFilter) )
	{
...
bool T1SFilterList::IsFrontendFilterInUse(TSharedRef<FT1FrontendFilter> FrontendFilter) const
{
	for ( auto FilterIt = Filters.CreateConstIterator(); FilterIt; ++FilterIt)
	{
		TSharedPtr<FT1FrontendFilter> Filter = (*FilterIt)->GetFrontendFilter();
...
void T1SFilterList::FrontendFilterCategoryClicked(TSharedPtr<FT1FrontendFilterCategory> MenuCategory)
{
...
bool T1SFilterList::IsFrontendFilterCategoryInUse(TSharedPtr<FT1FrontendFilterCategory> MenuCategory) const
{
	for ( auto FrontendFilterIt = AllFrontendFilters.CreateConstIterator(); FrontendFilterIt; ++FrontendFilterIt )
	{
		const TSharedRef<FT1FrontendFilter>& FrontendFilter = *FrontendFilterIt;
...
void T1SFilterList::OnResetFilters()
{
	RemoveAllFilters();
}
