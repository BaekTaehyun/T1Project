...
class FWidgetPath;
class ST1AssetColumnView;
class ST1AssetListView;
class ST1AssetTileView;
class SComboButton;
class UFactory;
struct FPropertyChangedEvent;

/** Delegate called when selection changed. Provides more context than FT1OnAssetSelected */
DECLARE_DELEGATE_TwoParams(FT1OnAssetSelectionChanged, const FAssetData& /*InAssetData*/, ESelectInfo::Type /*InSelectInfo*/);
 ...
 */
class ST1AssetView : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(ST1AssetView)
		: _AreRealTimeThumbnailsAllowed(true)
		, _ThumbnailLabel( EThumbnailLabel::ClassName )
		, _AllowThumbnailHintLabel(true)
		, _InitialViewType(ET1AssetViewType::Tile)
		...
		SLATE_EVENT( FT1OnShouldFilterAsset, OnShouldFilterAsset )

		/** Called to when an asset is selected */
		SLATE_EVENT( FT1OnAssetSelected, OnAssetSelected )

		/** Called to when an asset is selected. Provides more context OnAssetSelected */
		SLATE_EVENT( FT1OnAssetSelectionChanged, OnAssetSelectionChanged )

		/** Called when the user double clicks, presses enter, or presses space on an asset */
		SLATE_EVENT( FT1OnAssetsActivated, OnAssetsActivated )

		/** Called when an asset is right clicked */
		SLATE_EVENT( FT1OnGetAssetContextMenu, OnGetAssetContextMenu )

		/** Delegate to invoke when a context menu for a folder is opening. */
		SLATE_EVENT( FT1OnGetFolderContextMenu, OnGetFolderContextMenu )

		/** The delegate that fires when a path is right clicked and a context menu is requested */
		SLATE_EVENT( FT1ContentBrowserMenuExtender_SelectedPaths, OnGetPathContextMenuExtender )

		/** Invoked when a "Find in Asset Tree" is requested */
		SLATE_EVENT( FT1OnFindInAssetTreeRequested, OnFindInAssetTreeRequested )

		/** Called when the user has committed a rename of one or more assets */
		SLATE_EVENT( FT1OnAssetRenameCommitted, OnAssetRenameCommitted )
		...
		SLATE_ARGUMENT( FT1SourcesData, InitialSourcesData )
		...
		SLATE_ARGUMENT( ET1AssetViewType::Type, InitialViewType )
	...
	void SetSourcesData(const FT1SourcesData& InSourcesData);

	/** Returns the sources filter applied to this asset view */
	const FT1SourcesData& GetSourcesData() const;
	...
	void SyncTo( const FT1ContentBrowserSelection& ItemSelection, const bool bFocusOnSync = true );

	/** Sets the state of the asset view to the one described by the history data */
	void ApplyHistoryData( const FT1HistoryData& History );

	/** Returns all the items currently selected in the view */
	TArray<TSharedPtr<FT1AssetViewItem>> GetSelectedItems() const;
...
private:
	...
	TSharedRef<class ST1AssetTileView> CreateTileView();

	/** Creates a new list view */
	TSharedRef<class ST1AssetListView> CreateListView();

	/** Creates a new column view */
	TSharedRef<class ST1AssetColumnView> CreateColumnView();
	...
	void SetCurrentViewType(ET1AssetViewType::Type NewType);
	...
	void SetSelection(const TSharedPtr<FT1AssetViewItem>& Item);

	/** Sets selection for an item in all lists in the view */
	void SetItemSelection(const TSharedPtr<FT1AssetViewItem>& Item, bool bSelected, const ESelectInfo::Type SelectInfo = ESelectInfo::Direct);

	/** Scrolls the selected item into view for all lists in the view */
	void RequestScrollIntoView(const TSharedPtr<FT1AssetViewItem>& Item);

	/** Handler for list view widget creation */
	TSharedRef<ITableRow> MakeListViewWidget(TSharedPtr<FT1AssetViewItem> AssetItem, const TSharedRef<STableViewBase>& OwnerTable);

	/** Handler for tile view widget creation */
	TSharedRef<ITableRow> MakeTileViewWidget(TSharedPtr<FT1AssetViewItem> AssetItem, const TSharedRef<STableViewBase>& OwnerTable);

	/** Handler for column view widget creation */
	TSharedRef<ITableRow> MakeColumnViewWidget(TSharedPtr<FT1AssetViewItem> AssetItem, const TSharedRef<STableViewBase>& OwnerTable);

	/** Handler for when any asset item widget gets destroyed */
	void AssetItemWidgetDestroyed(const TSharedPtr<FT1AssetViewItem>& Item);
	...
	TSharedPtr<class FAssetThumbnail> AddItemToNewThumbnailRelevancyMap(const TSharedPtr<FT1AssetViewAsset>& Item, TMap< TSharedPtr<FT1AssetViewAsset>, TSharedPtr<class FAssetThumbnail> >& NewRelevantThumbnails);

	/** Handler for tree view selection changes */
	void AssetSelectionChanged( TSharedPtr<FT1AssetViewItem > AssetItem, ESelectInfo::Type SelectInfo );

	/** Handler for when an item has scrolled into view after having been requested to do so */
	void ItemScrolledIntoView(TSharedPtr<FT1AssetViewItem> AssetItem, const TSharedPtr<ITableRow>& Widget);
	...
	void OnListMouseButtonDoubleClick(TSharedPtr<FT1AssetViewItem> AssetItem);
	...
	bool AssetVerifyRenameCommit(const TSharedPtr<FT1AssetViewItem>& Item, const FText& NewName, const FSlateRect& MessageAnchor, FText& OutErrorMessage);

	/** An asset item has started to be renamed */
	void AssetRenameBegin(const TSharedPtr<FT1AssetViewItem>& Item, const FString& NewName, const FSlateRect& MessageAnchor);
	...
	void AssetRenameCommit(const TSharedPtr<FT1AssetViewItem>& Item, const FString& NewName, const FSlateRect& MessageAnchor, const ETextCommit::Type CommitType);
	 ...
	 */
	UObject* CreateAssetFromTemporary(FString InName, const TSharedPtr<FT1AssetViewAsset>& InItem, FText& OutErrorText);
...
public:
	...
	FT1OnFolderPathChanged OnFolderPathChanged;
...
private:
	...
	TArray<TSharedPtr<FT1AssetViewItem>> FilteredAssetItems;
	...
	ET1AssetViewType::Type CurrentViewType;
	TSharedPtr<class ST1AssetListView> ListView;
	TSharedPtr<class ST1AssetTileView> TileView;
	TSharedPtr<class ST1AssetColumnView> ColumnView;
	...
	FT1SelectionData PendingSyncItems;
	...
	FT1OnFindInAssetTreeRequested OnFindInAssetTreeRequested;

	/** Called when the user has committed a rename of one or more assets */
	FT1OnAssetRenameCommitted OnAssetRenameCommitted;
	...
	TWeakPtr<struct FT1AssetViewItem> AwaitingRename;

	/** Set when the user is in the process of naming an asset */
	TWeakPtr<struct FT1AssetViewItem> RenamingAsset;

	/** Pool for maintaining and rendering thumbnails */
	TSharedPtr<class FAssetThumbnailPool> AssetThumbnailPool;

	/** A map of FT1AssetViewAsset to the thumbnail that represents it. Only items that are currently visible or within half of the FilteredAssetItems array index distance described by NumOffscreenThumbnails are in this list */
	TMap< TSharedPtr<FT1AssetViewAsset>, TSharedPtr<class FAssetThumbnail> > RelevantThumbnails;

	/** The set of FAssetItems that currently have widgets displaying them. */
	TArray< TSharedPtr<FT1AssetViewItem> > VisibleItems;
	...
	FT1AssetViewSortManager SortManager;
