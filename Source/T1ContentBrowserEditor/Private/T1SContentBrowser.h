...
class FT1AssetContextMenu;
class FT1FrontendFilter_Text;
class FT1PathContextMenu;
class FTabManager;
...
public:
	...
	SLATE_BEGIN_ARGS(ST1ContentBrowser)
		: _ContainingTab()
	...
	void Construct( const FArguments& InArgs, const FName& InstanceName, const FT1ContentBrowserConfig* Config );
	 ...
	 *								  false to force the view to select the explicit Parent folders of each asset 
	 */
	void SyncTo( const FT1ContentBrowserSelection& ItemSelection, const bool bAllowImplicitSync = false, const bool bDisableFiltersThatHideAssets = true );
...
private:
	...
	void OnApplyHistoryData(const FT1HistoryData& History);

	/** Updates the supplied history data with current information */
	void OnUpdateHistoryData(FT1HistoryData& History) const;
	...
	TSharedPtr<SWidget> GetFolderContextMenu(const TArray<FString>& SelectedPaths, FT1ContentBrowserMenuExtender_SelectedPaths InMenuExtender, FT1OnCreateNewFolder OnCreateNewFolder, bool bPathView);
	...
	void CreateNewFolder(FString FolderPath, FT1OnCreateNewFolder OnCreateNewFolder);
...
private:
	...
	FT1HistoryManager HistoryManager;

	/** A helper class to manage asset context menu options */
	TSharedPtr<class FT1AssetContextMenu> AssetContextMenu;

	/** The context menu manager for the path view */
	TSharedPtr<class FT1PathContextMenu> PathContextMenu;
	...
	TSharedPtr<ST1AssetView> AssetViewPtr;
	...
	/** The filter list */
	TSharedPtr<T1SFilterList> FilterListPtr;
	...
	TSharedPtr< FT1FrontendFilter_Text > TextFilter;

	/** Commands handled by this widget */
	TSharedPtr< FUICommandList > Commands;

	/** Delegate used to create a new folder */
	FT1OnCreateNewFolder OnCreateNewFolder;
