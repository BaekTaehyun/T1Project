 ...
 */
enum class ET1NativeClassHierarchyNodeType : uint8
{
	Folder,
	Class,
};
 ...
 */
struct FT1NativeClassHierarchyPluginModuleInfo
{
	/** Name of the module*/
	FName Name;
 ...
 */
struct FT1NativeClassHierarchyNodeKey
{
	FT1NativeClassHierarchyNodeKey(const FName InName, const ET1NativeClassHierarchyNodeType InType)
		: Name(InName)
		, Type(InType)
	{
	}

	FORCEINLINE bool operator==(const FT1NativeClassHierarchyNodeKey& Other) const
	{
	...
	friend FORCEINLINE uint32 GetTypeHash(const FT1NativeClassHierarchyNodeKey& InKey)
	{
		uint32 KeyHash = 0;
	...
	/** Type of this entry */
	ET1NativeClassHierarchyNodeType Type;
};

/**
 * Single node in the class hierarchy
 */
struct FT1NativeClassHierarchyNode
{
	/** Helper function to make a folder node entry */
	static TSharedRef<FT1NativeClassHierarchyNode> MakeFolderEntry(FName InEntryName, FString InEntryPath);

	/** Helper function to make a class node entry */
	static TSharedRef<FT1NativeClassHierarchyNode> MakeClassEntry(UClass* InClass, FName InClassModuleName, FString InClassModuleRelativePath, FString InEntryPath);

	void AddChild(TSharedRef<FT1NativeClassHierarchyNode> ChildEntry);
	...
	TMap<FT1NativeClassHierarchyNodeKey, TSharedPtr<FT1NativeClassHierarchyNode>> Children;
 ...
 */
struct FT1NativeClassHierarchyFilter
{
	...
	FT1NativeClassHierarchyFilter()
	{
		bRecursivePaths = false;
	}

	/** Appends the other filter to this one */
	void Append(const FT1NativeClassHierarchyFilter& Other)
	{
...
public:
	 ...
	 */
	void GetMatchingClasses(const FT1NativeClassHierarchyFilter& Filter, TArray<UClass*>& OutClasses) const;
	 ...
	 */
	void GetMatchingFolders(const FT1NativeClassHierarchyFilter& Filter, TArray<FString>& OutFolders) const;
...
private:
	 ...
	 */
	static bool HasClassesRecursive(const TSharedRef<FT1NativeClassHierarchyNode>& HierarchyNode, const bool bRecurse = true);
	 ...
	 */
	static bool HasFoldersRecursive(const TSharedRef<FT1NativeClassHierarchyNode>& HierarchyNode, const bool bRecurse = true);
	 ...
	 * @param bRecurse - True to recurse into sub-folders, false to only check the given node
	 */
	static void GetClassesRecursive(const TSharedRef<FT1NativeClassHierarchyNode>& HierarchyNode, TArray<UClass*>& OutClasses, const bool bRecurse = true);
	 ...
	 * @param bRecurse - True to recurse into sub-folders, false to only check the given node
	 */
	static void GetFoldersRecursive(const TSharedRef<FT1NativeClassHierarchyNode>& HierarchyNode, TArray<FString>& OutFolders, const bool bRecurse = true);
	 ...
	 */
	void GatherMatchingNodesForPaths(const TArrayView<const FName>& InClassPaths, TArray<TSharedRef<FT1NativeClassHierarchyNode>, TInlineAllocator<4>>& OutMatchingNodes) const;
	 ...
	 */
	void AddClass(UClass* InClass, const TSet<FName>& InGameModules, const TMap<FName, FT1NativeClassHierarchyPluginModuleInfo>& InPluginModules, FAddClassMetrics& AddClassMetrics);
	 ...
	 */
	static FName GetClassPathRootForModule(const FName& InModuleName, const TSet<FName>& InGameModules, const TMap<FName, FT1NativeClassHierarchyPluginModuleInfo>& InPluginModules, EPluginLoadedFrom& OutWhereLoadedFrom);
	 ...
	 */
	static TMap<FName, FT1NativeClassHierarchyPluginModuleInfo> GetPluginModules();

	/** Root level nodes corresponding to the root folders used by the Content Browser, eg) Classes_Engine, Classes_Game, etc */
	TMap<FName, TSharedPtr<FT1NativeClassHierarchyNode>> RootNodes;
