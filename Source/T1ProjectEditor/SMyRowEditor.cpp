...
#define LOCTEXT_NAMESPACE "SMyRowEditor"
...
void SMyRowEditor::NotifyPreChange( UProperty* PropertyAboutToChange )
{
...
void SMyRowEditor::NotifyPostChange( const FPropertyChangedEvent& PropertyChangedEvent, UProperty* PropertyThatChanged )
{
	check(DataTable.IsValid());
...
void SMyRowEditor::PreChange(const class UUserDefinedStruct* Struct, FStructureEditorUtils::EStructureEditorChangeInfo Info)
{
...
void SMyRowEditor::PostChange(const class UUserDefinedStruct* Struct, FStructureEditorUtils::EStructureEditorChangeInfo Info)
{
	if (Struct && (GetScriptStruct() == Struct))
	{
		RefreshNameList();
...
void SMyRowEditor::PreChange(const UDataTable* Changed, FDataTableEditorUtils::EDataTableChangeInfo Info)
{
...
void SMyRowEditor::PostChange(const UDataTable* Changed, FDataTableEditorUtils::EDataTableChangeInfo Info)
{
	if ((Changed == DataTable.Get()) && (FDataTableEditorUtils::EDataTableChangeInfo::RowList == Info))
	{
		RefreshNameList();
...
void SMyRowEditor::CleanBeforeChange()
{
...
void SMyRowEditor::RefreshNameList()
{
...
UScriptStruct* SMyRowEditor::GetScriptStruct() const
{
	return DataTable.IsValid() ? DataTable->RowStruct : NULL;
}

FName SMyRowEditor::GetCurrentName() const
{
	return SelectedName.IsValid() ? *SelectedName : NAME_None;
}

FText SMyRowEditor::GetCurrentNameAsText() const
{
	return FText::FromName(GetCurrentName());
}

FString SMyRowEditor::GetStructureDisplayName() const
{
...
TSharedRef<SWidget> SMyRowEditor::OnGenerateWidget(TSharedPtr<FName> InItem)
{
	return SNew(STextBlock).Text(FText::FromName(InItem.IsValid() ? *InItem : NAME_None));
}

void SMyRowEditor::OnSelectionChanged(TSharedPtr<FName> InItem, ESelectInfo::Type InSeletionInfo)
{
	if (InItem.IsValid() && InItem != SelectedName)
	{
		...
		if (RenameTextBox.IsValid())
		{
			...
			RenameTextBox->SetText(TAttribute<FText>(this, &SMyRowEditor::GetCurrentNameAsText));
		}
	}
}

void SMyRowEditor::SelectRow(FName InName)
{
...
void SMyRowEditor::HandleUndoRedo()
{
	RefreshNameList();
	Restore();
}

FReply SMyRowEditor::OnAddClicked()
{
	if (DataTable.IsValid())
	{
...
FReply SMyRowEditor::OnMoveRowClicked(FDataTableEditorUtils::ERowMoveDirection MoveDirection)
{
	if (DataTable.IsValid())
	{
...
FReply SMyRowEditor::OnMoveToExtentClicked(FDataTableEditorUtils::ERowMoveDirection MoveDirection)
{
	if (DataTable.IsValid())
	{
...
void SMyRowEditor::OnRowRenamed(const FText& Text, ETextCommit::Type CommitType)
{
...
FReply SMyRowEditor::OnResetToDefaultClicked()
{
...
EVisibility SMyRowEditor::GetResetToDefaultVisibility() const
{
...
void SMyRowEditor::Construct(const FArguments& InArgs, UDataTable* Changed)
{
	DataTable = Changed;
	{
